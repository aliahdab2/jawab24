...
## D-104 · Salla billing is verify-first like Zid: webhooks trigger, `GET /apps/{app_id}/subscriptions` decides, entitlement derived from `end_date` (2026-08-26)

**Context.** D-103 put two paid plans on the Salla listing, which activates apps-policy
Article 5 (Salla owns the money) — and no code path existed from an `app.subscription.*`
delivery to a Jawab24 plan. Salla, like Zid (D-070) and unlike Shopify, documents a
subscription-read endpoint: `GET https://api.salla.dev/admin/v2/apps/{app_id}/subscriptions`
(merchant token, docs.salla.dev 5401098e0). Unlike Zid's, its payload carries **no status
field** — only `plan_name`/`plan_type`/`start_date`/`end_date`/`price` per entry, mixing the
base plan with add-ons (`item_type`) — and base plans carry **no plan id** with a nullable
`plan_name`.

**Ruling.** The Salla rail is **verify-first** (`services/sallaBilling.ts`):
`app.subscription.*`/`app.trial.*` deliveries and the post-claim hook carry no state — they
only trigger `syncSallaBilling(storeId)`, which reads the API and reconciles the local mirror
(`payment_method='salla'`, `salla_store_id`, migration 0181); a 6-hourly
`SallaBillingReconcile` is the authority of last resort. Entitlement is DERIVED from
`end_date`: future = entitled (null price = the trial window), past = inactive, and a missing
or unparseable date is `unknown_state` — fail loud, write nothing, exactly the direction
D-070 fixed for Zid's unknown status (a stale entitlement costs us a little money; a wrongly
revoked one costs a paying customer). Plans map name-first («الأعمال»/«الاحترافي»,
normalizeArabic-folded) with the D-103 ex-VAT prices (146/296) as the fallback identity —
prices are usable as keys only because D-103 made them distinct; `plan_type: 'free'` is a
known non-entitling shape; anything else unmapped is `unknown_plan`, fail-loud, no activation.

**Consequences.** The Stripe suppression fires on the mirror row itself (`isSallaBilled`,
canceled rows exempt) with the store-presence heuristic kept as the pre-subscription fallback;
`SALLA_APP_ID` (665811310) must be in prod env before the listing submits, or the rail is
dormant and a paying reviewer books nothing. The D-103 "parked" question — how a Salla-billed
subscription reconciles with Stripe-side entitlement — is answered by this design: one
subscriptions row per subject, cross-rail adopts refuse over any live paid row (Sentry, human
decides). Every envelope field is [provisional] until the first live paid subscription; the
plan NAMES AND PRICES in the portal wizard are load-bearing for the mapping — renaming or
repricing there without updating `config/sallaBilling.ts` books `unknown_plan` for every
paying merchant on that plan.

**Rejected.** (a) Webhook-driven state (parse the delivery payload): the envelope is
uncaptured and add-on events share the same names — a misread payload writes wrong billing
state; a misread trigger only costs a spurious verify. (b) Trusting `plan_name` alone: the
...
**Why URLs are excluded.** The marks are invisible on screen but they travel with a copy-paste.
Inside a product link that breaks the URL — and the reply that carries a store link is the reply
that was going to become an order.
...
means calling `isolateNumericTokens` at those send sites too.

**Evidence.** `packages/shared/src/__tests__/bidi.test.ts` pins every row of the table above
(measurement-derived, mutation-checked against 8 mutations: isolation removed, fragile guard
removed, URL protection removed, idempotence guard removed, LRI/PDI swapped, `stripBidiMarks`
no-op'd, newline-straddling separators, RTL guard removed — each caught). jsdom does no bidi

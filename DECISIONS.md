...
## D-082 · The merchant says what the post is about and what the picture shows — two boxes, and the gap is named rather than filled

**⚠️ Numbering note.** An earlier, abandoned branch recorded this work as "D-075/D-076". Those numbers were never merged and belong to nothing on `main`; D-081 is claimed by the open contact-standard PR. This entry is the only ruling on the subject.

**Two free-text boxes replace the five angle chips.** «عمَّ تريد المنشور؟» steers what the post SAYS; «وصف الصورة» steers what the picture SHOWS. Both optional, both capped at `POST_SUGGESTION_BRIEF_MAX` (500), both stored on the row (migration 0165) because generation finishes in a worker long after the request returned.

**Why the chips went (owner ruling, 2026-08-13):** «انا شايف مالها لازمة لانه في مكان تكتب شو بدك». A free-text request is a strictly more expressive duplicate of a preset angle, and the request already outranks the angle server-side. The five `type_*` labels stay — the dashboard card still names the angle a post was written on — but `tryAngle`, `angleNeedsData` and `angleNeedsDataHint` are gone with the UI they described. The SERVER still accepts and honours `postType` for shipped mobile bundles that predate this; only our client stopped sending it.

**Why TWO boxes and not one.** They steer different halves and are routinely wanted apart — «صورة فيها صبية محجبة حاملة كراس» says nothing about what the caption should argue. Folded into a single box the scene description competes with the subject for the model's attention.

**Ground-and-flag, not fill.** A request NAMES a subject; it is not evidence the subject exists. On 2026-08-12 an appliance importer asked for a post about an English language course and got one, complete with an invented course line, on a page whose catalogue is televisions. Nothing in the data supported it and nothing said so. The rule is now the settled standard for grounded generation: never assert what the retrieved context does not carry, and **disclose** the gap rather than fill it. `unmet_request` holds the disclosure in Arabic, in the merchant's own terms; the sheet renders it with a link to Business Info. Refusing to invent is only half an answer — silently writing about something else reads as the box ignoring them.

**People are allowed when — and only when — the IMAGE box asks for them.** Meta labels a photorealistic AI person more prominently than a scene (re-verified 2026-08-12; the documented rule is strongest for ADS, and these posts are organic, so the penalty is real but milder than the original code comment claimed). That trade — a visible AI label in exchange for the picture they asked for — belongs to the merchant, not to us. Substituting a desk of notebooks for the person they described is not a safer answer, it is an unasked-for one. The fence that stays is ours to keep: anonymous, non-identifiable people only, never a named or real individual, never a public figure, never a child. A requested person also forces `photo` mode, because a poster is typography drawn in code and a conceptual shot is materials — neither can hold a person, and leaving the rotation in charge would grant people in the prompt and then silently drop them one day in three.

**`image_refused` is split from `image_failed`.** They ask different things of the merchant: a refusal is the provider declining the scene THEY described and only they can rewrite it, while a failure is ours and retrying may work. Classified from the error's status and code, **never its message** — provider copy is not an API contract, and this repo has already shipped one classifier that matched on error text and silently stopped firing.

**Figures now carry provenance instead of a flat verdict.** Once merchants can type a request, "ungrounded" is wrong in both directions: exclude the request and every price they type themselves is logged as invented until the log is unreadable; merge it into the data and the guard goes blind exactly where invention is likeliest. `classifyFigures` splits `invented` (the only real alarm, WARN) from `fromRequest` (their own assertion, INFO). Still log-only, still token equality, never substring. Only the TEXT box can credit a caption figure — a scene description is about the picture. The tokeniser also folds Arabic separators (U+066C/U+066B) and scale words («٥٠ ألف» → 50000), without which a correctly grounded Arabic price read as invented — a false positive on the exact input this feature exists for.

**The unrequested path is byte-identical to what shipped.** Every request-only clause, extra return field and people-rule variation appears only when its own box was filled. A merchant who ignores both fields gets the prompt that existed before this change; that is what makes the change safe to ship to the whole pilot at once, and it is pinned by a test.

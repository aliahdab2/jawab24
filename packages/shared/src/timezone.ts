...
/**
 * Is `now` inside the merchant's business hours?
 *
 * THE definition — the reply pipeline and the settings UI both call this, so the
 * "Active / Outside Hours" badge can never disagree with what the pipeline does.
 * They previously disagreed: the backend treated `end` as inclusive and the card
 * as exclusive, so at exactly 18:00 the bot replied while the UI said it was closed.
 *
 * `end` is EXCLUSIVE, the conventional reading of a 09:00–18:00 working day: at
 * 18:00 the day is over. An empty or inverted window (end <= start) is never open.
 */
...
export function utcOffsetMinutes(timeZone: string, now: Date = new Date()): number {
    const match = /^UTC([+-])(\d{2}):(\d{2})$/.exec(formatUtcOffset(timeZone, now));
    if (!match) return 0;
...
export function getTimezoneOptions(ensure: (string | undefined)[] = [], now: Date = new Date()) {
    ...
    return [...zones]
        .map(value => ({ value, offset: formatUtcOffset(value, now) }))
        .sort((a, b) =>
            utcOffsetMinutes(a.value, now) - utcOffsetMinutes(b.value, now)

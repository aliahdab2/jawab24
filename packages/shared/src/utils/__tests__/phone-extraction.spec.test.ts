 ...
 * extractor BEFORE the implementation exists, so it is expected to be RED until
 * `extractPhones` and `countryFromTimezone` are implemented in ../validation.
 *
 ...
 *   function countryFromTimezone(tz?: string | null): string | undefined;
 */
import { describe, it, expect } from 'vitest';
// These imports are intentionally ahead of the implementation (test-first).
import { extractPhones, extractPhoneFromText, countryFromTimezone } from '../validation';

const e164s = (text: string, defaultCountry?: string) =>
  extractPhones(text, defaultCountry ? { defaultCountry } : undefined).map(p => p.e164);

describe('countryFromTimezone — merchant region inference (from settings.timezone)', () => {
  it('maps the timezones present in prod', () => {
  ...
  it('maps other Arabic-market timezones we serve', () => {
  ...
  it('returns undefined for missing/unknown timezone (extractor then handles +CC + fallback only)', () => {

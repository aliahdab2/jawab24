...
/**
 * Gates ALL phone UI: the login phone tab, the sidebar phone name-fallback, and
 * the (dormant) phone-collect page.
 *
 * ⛔ OFF, and turning it on delivers nothing: there is NO verification transport
 * (D-123). The SMS rail was removed with its Vonage provider, and WhatsApp OTP
 * needs a Jawab24-owned WhatsApp Business Account plus an AUTHENTICATION-category
 * template — neither exists. The backend answers 503 `otp_unavailable`.
 * Onboarding never forces a phone regardless of this flag; that is decoupled at
 * the code level (see backend/src/controllers/auth.ts).
 *
 * Prerequisites for switching it on: `.planning/WHATSAPP_PLAN.md`. Syria stays
 * exempt whatever the transport (@jawab24/shared `SANCTIONED_DIAL_PREFIXES`).
 */

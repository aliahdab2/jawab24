...
/**
 * Card showing reply activity broken down by method (AI vs template vs manual).
...
export function RepliesSection({ replies }: { replies: EcommerceAnalyticsOverview['replies'] }) {
    ...
    const max = Math.max(replies.aiReplies, replies.templateReplies, replies.manualReplies, 1);
    ...
    return (
        <Card className="border-none shadow-[0_4px_16px_rgba(0,0,0,0.04)] p-5 landscape:p-4">
            ...
            <div className="space-y-3">
                ...
                <HorizontalBar label={t('replies.template')} value={replies.templateReplies} max={max} />

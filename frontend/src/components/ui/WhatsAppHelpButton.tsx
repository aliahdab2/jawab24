...
export function WhatsAppHelpButton({ hidden = false }: { hidden?: boolean }) {
  ...
  return (
    <>
      ...
      {/* Popup card - Full screen overlay in landscape for better UX */}
      {isOpen && (
        <>
          {/* Backdrop for landscape - clicking outside closes */}
          {isLandscape && (
            <div 
          ...
          <div
            className={clsx(
              "fixed z-50 bg-white shadow-2xl border border-surface-100 overflow-hidden animate-slide-up",
              isLandscape 
                // Landscape: Centered modal with safe area padding
                ? "top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-3xl p-6 w-[90vw] max-w-md"
                // Portrait: Corner popup above button
                : "rounded-[2.5rem] p-8 w-80 max-w-[calc(100vw-4rem)]"
            )}
            style={!isLandscape ? {
          ...
          >
            ...
            <div className={clsx("text-center", isLandscape ? "mb-4" : "mb-6")}>
              <div className={clsx(
                "bg-emerald-100 rounded-2xl flex items-center justify-center mx-auto shadow-inner",
                isLandscape ? "w-14 h-14 mb-3" : "w-16 h-16 mb-4"
              )}>
                <MessageCircle className={clsx("text-emerald-600", isLandscape ? "w-7 h-7" : "w-8 h-8")} />
              </div>
              <h3 className={clsx(
                "font-display font-bold text-surface-900 tracking-tight",
                isLandscape ? "text-lg" : "text-xl"
              )}>
                {t('common.needHelp')}
              </h3>
              <p className={clsx(
                "font-medium text-surface-500 leading-relaxed",
                isLandscape ? "text-xs mt-1" : "text-sm mt-2"
              )}>
            ...
            <button
              ...
              className={clsx(
                "w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-2xl flex items-center justify-center gap-3 transition-all shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/30 active:scale-95",
                isLandscape ? "py-3 px-5" : "py-4 px-6"
              )}
            >
              <svg className={clsx(isLandscape ? "w-5 h-5" : "w-6 h-6")} fill="currentColor" viewBox="0 0 24 24">
            ...
            <p className={clsx(
              "font-bold text-surface-400 text-center uppercase tracking-widest",
              isLandscape ? "text-[9px] mt-3" : "text-[10px] mt-4"
            )}>

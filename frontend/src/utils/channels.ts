 ...
 *
 * This gates the per-conversation channel ribbon on inbox rows. Connection alone
 ...
 * multi-channel and get a ribbon on every row. A channel that's linked but paused
 * (auto-reply off) doesn't count — if only one channel is live, the channel is
 * unambiguous and the ribbon is just noise.
 */

...
| `likeComments` | commentProcessor (smart path only) | Page likes the customer's comment after a successful smart reply. Default off. Facebook only (IG Graph has no like endpoint — the IG adapter simply lacks `likeComment`). Suppressed when the reply is flagged (`needsAttention`) or intent is `COMPLAINT` / `OFFENSIVE` / `SPAM_OR_IRRELEVANT`. Post Reply is NOT governed by this — it has the per-post `posts.like_comment` toggle. |

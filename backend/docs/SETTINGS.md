...
| `replyMode` | prompt builder + `ai.ts` context + both reply caches + lead alerts | `sales` (default — today's behavior) / `info` (information desk: the INFO-DESK MODE per-call prompt block overrides the static prompt's contact-asking / follow-up-promise demonstrations; lead capture stays passive with push alerts suppressed). **Workspace default with a per-page override**: `pages.reply_mode` (NULL = inherit), resolved via `resolveEffectiveReplyMode` in `@jawab24/shared` and written via `PATCH /pages/:id/reply-mode`. Cache scoping is read-side — conditional `rm:i` exact-key segment + semantic-cache `replyMode` metadata (strict both-ways, legacy rows = sales) — so a mode toggle takes effect immediately with no version bump. `'info'` is allowlist-gated at the write path while piloting (`config.replyMode.workspaceIds`, empty = GA). See D-083. |

    ...
    async createPlan(data: Partial<Plan>): Promise<Plan> {
        const result = await db
            ...
            .values({
                ...
                maxTemplates: data.maxTemplates ?? 3,
                maxRules: data.maxRules ?? 2,
                facebookEnabled: data.facebookEnabled ?? true,
                instagramEnabled: data.instagramEnabled ?? true,
                whatsappEnabled: data.whatsappEnabled ?? false,
                ecommerceEnabled: data.ecommerceEnabled ?? false,
    ...
    async updatePlan(planId: string, data: Partial<Plan>): Promise<Plan | null> {
        const result = await db
            ...
            .set({
                ...
                ...(data.maxTemplates !== undefined && { maxTemplates: data.maxTemplates }),
                ...(data.maxRules !== undefined && { maxRules: data.maxRules }),

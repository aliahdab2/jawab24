 ...
 * rather than free-text RAG chunks.
 *
 * Used to surface non-blocking warnings in the editor (Stage 1.3) and as the
 * input signal for the Stage 3 "Restructure my KB" extraction flow.
 *
...
export function detectCatalogLikePatterns(text: string): CatalogDetection {
    ...
    const priceMatches = [
        ...text.matchAll(NUMBER_BEFORE_CURRENCY_AR),
        ...text.matchAll(NUMBER_BEFORE_CURRENCY_EN),
        ...text.matchAll(SYMBOL_BEFORE_NUMBER),
    ];
    const priceCount = priceMatches.length;

    const courseMatches = [...text.matchAll(COURSE_KEYWORDS)];

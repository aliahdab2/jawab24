...
describe('ReplyGenerator - Playground tool-loop routing', () => {
    ...
    // The playground/eval must see what a customer sees — the same rendering the
    // live adapter applies before sending (PROD 2026-08-23: `![فستان](<page>)`
    // reached Messenger literally). Target comes from the page's channels.
    it('renders the reply for the playground\'s render target, plain by default', async () => {
        const { aiService } = await import('../../src/services/ai');
        vi.mocked(aiService.generateReply).mockResolvedValue({
            ...mockAiResponse,
            reply: '**تنورة** — [اطلبيها](https://demostore.salla.sa/dev-x/تنورة/p812874023)',
        });

        const plain = await generator.generateForPlayground(baseInput);
        expect(plain.reply).toBe('تنورة — اطلبيها: https://demostore.salla.sa/dev-x/تنورة/p812874023');

        const wa = await generator.generateForPlayground({ ...baseInput, renderTarget: 'whatsapp' });
        expect(wa.reply).toBe('*تنورة* — اطلبيها: https://demostore.salla.sa/dev-x/تنورة/p812874023');
    });
});

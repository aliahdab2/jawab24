...
- **Attribution is manual-only today** (admin dropdown). A future referral link (`?ref=<code>`) stamps the same `users.partner_id` at signup — planned, ❌ NOT IMPLEMENTED.
- **Payments ledger** (`payments`, migration 0171): the single record of every dollar received, whatever rail it arrived on — Stripe, cash, Sham Cash, bank transfer. It replaces the previous gap where manual revenue wrote no row at all and production Syria revenue therefore read 0.
  - **Writers**: the reseller portal (`POST /partner/merchants/:userId/payments` — the portal's ONLY mutation, behind the same ownership gate, `collectedBy` forced to `partner`), the admin console (`POST /admin/users/:userId/payments`, with a "a reseller collected this" toggle), and the Stripe webhook (`invoice.payment_succeeded` → `recordStripePayment`, keyed on the invoice id so replays book nothing twice, and non-throwing so a bookkeeping failure can never break subscription activation).
  - **Settlement**: money a rep holds is `recorded` (outstanding); Stripe/admin money is `settled` on arrival. `POST /admin/payments/:id/settle` is the handover (تسليم المبلغ); `POST /admin/payments/:id/void` retires a mistake **without deleting the row**. A DB CHECK keeps `status='settled'` and `settled_at` agreeing in both directions.
  - **Commission** is snapshotted per row (`commission_pct` + `commission_cents`) at record time, so changing a rep's rate never rewrites what he already handed over. ⛔ **Neither field is projected to the partner surface** (owner ruling 2026-08-16): the portal receives gross + `netOwedCents` only, so the rate cannot be recovered by subtraction in the browser. `test/services/partnerMerchantDetail.test.ts` pins this.
  - **"Who hasn't paid"** is derived, never stored: `isUnpaid` flags a merchant only when the billed period has ENDED and no payment covers past it (`past_due` always counts; a trial never does). Surfaced as the `مستحق` chip + filter in `/partner` and the `statUnpaid` count.
  - **Backfill**: `backend/scripts/backfill-payments-ledger.ts` books paid `payment_requests` and succeeded `topup_purchases` (dry-run by default, `--apply` to write, idempotent via `backfill:<table>:<id>` keys). Stripe subscription invoices from BEFORE the deploy are deliberately not backfilled — that history lives in Stripe, and synthesising rows from `current_period_start` would fabricate payments that may never have happened.

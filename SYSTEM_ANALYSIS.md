...
- **Non-text types handled**: voice (transcribed), image (vision), video/file (nudge), sticker (silent), Instagram story mentions `story_mention` / `ig_story` (silent + auto-resolved — the customer asked nothing, and one resort's entire `[مرفق]` volume was story tags being nudged and then filed as unanswered)

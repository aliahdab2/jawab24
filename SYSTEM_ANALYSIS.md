...
| 12 | Facts buried at the tail of a long KB are ignored | High | When a merchant's Business Info is dominated by a long repetitive list (retail outlets, branches, distributor networks), a short price/policy section near the END is answered as "not available" **even though it is in the prompt**. Explicitly **not** a retrieval bug: non-ecommerce pages bypass RAG and receive the full KB verbatim (`generator.ts:747-769`, D-012), and the KB was under the 16k `KB_MAX_CHARS` cap so it wasn't truncated either — the prices were in every prompt. Found live on BAMBO LIBYA (2026-07-27): every «رقم 5 بقداش» returned «ما عندي الأسعار الدقيقة» while the merchant's own Post Reply auto-DM quoted the price on the same page. The price lines sat at char 9,730 of 10,641 (91% depth) behind ~6,700 chars of ~200 near-identical «صيدلية X - district» lines — a lost-in-the-middle attention failure amplified by low-entropy distractor text. Regression coverage: playground-eval Cat 69 (#724–#727) + the `distributor` fixture (burial depth 89%/97% vs prod 91%/96%). Candidate fixes, best first: promote price lists to structured `catalog_items` (the `<product_catalog>` block is compact, always injected, and carries explicit authority — makes burial structurally impossible); or hoist merchant price sections above long list sections at prompt-assembly time |
| 13 | No validator for place / name claims | High | Check 1 grounds **numbers** only. A reply can attribute real KB entities to a location that appears nowhere in the KB and nothing flags it — the merchant never sees it in Needs Attention. Found live on BAMBO LIBYA (2026-07-27): asked about العجيلات (absent from both outlet lists), the AI returned the الزاوية list under «أما للعجيلات تحديداً فهذه هي الصيدليات المتوفرة», and repeated it after the customer objected «هدوم مش في العجيلات». Real names, invented city — indistinguishable from a correct answer to the merchant. Regression coverage: playground-eval Cat 69 (#728 fabrication, #729 green guard) |

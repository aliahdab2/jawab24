...
**Sort order is merchant-controlled (August 2026):** `GET /leads?sort=newest|oldest` (validated by the shared `parseLeadSortOrder`; an unrecognised value falls back rather than 400s). One toggle in the controls row flips the queue, persisted per browser in `leads-sort-oldest-first`. Default stays `newest` — flipping it fleet-wide would silently reorder every merchant's list — but `oldest` is the triage view, because newest-first structurally buries a neglected lead deeper with every new capture. Ordering is server-side by necessity: the list paginates 50 rows at a time, so a client-side sort would only reorder the loaded page. The `ORDER BY` carries `leads.id` as a tiebreaker; leads captured in the same second (a comment burst on one post) have equal `created_at`, and without a deterministic second key offset pagination can repeat or skip a row between page queries.

**Contacting a lead records progress (August 2026):** tapping Call or WhatsApp — in the detail panel, or the phone icon on a list row/card — advances a `new` lead to `contacted` (`shouldAdvanceOnContact`, shared so the UI and its tests assert one rule). It only ever advances OUT of `new`: the pipeline never regresses a lead the merchant already moved on, and a `new` lead can't carry `needs_follow_up` (guarded in the upsert), so the flag `updateLeadStatus` clears costs nothing here. Rationale: on 2026-08-05, of 17 pages holding leads only 3 had ever changed a lead's status — the other 14 sat at 100% `new`, so bookkeeping nobody performs had to become a by-product of work merchants already do. Feedback differs by surface *by design*: from the list a toast carries an Undo; from the panel there is **no** toast, because toasts sit at `z-45`, below every modal tier (a deliberate choice in `globals.css` — sonner's default painted them over an open sheet's sticky footer and blocked Save/Cancel), so one fired behind the open panel would be both invisible and unclickable. There the `StatusPicker` is the feedback and the one-tap undo; its selected segment now carries `aria-pressed` (it was distinguished by colour alone, WCAG 4.1.2). Covered by `frontend/e2e/leads.spec.ts` ("contact records progress"), including a hit-test of the Undo button — `toBeVisible` passes for an element buried under a portal, and that false pass is what hid the panel-side problem.

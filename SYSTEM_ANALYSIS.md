...
| **Revoked credential, caught live** | `NULL` (auto-reply untouched) | **`''` (cleared)** | set | `services/pageTokenRecovery.ts`, since 2026-08-14. The FIRST Graph call that fails with a token-revoked code (190/458·459·460·463, 200/10) re-mints the page token from `/me/accounts`; if the merchant's own session is gone too, the page is marked disconnected and the merchant is told **which cause** it was (in-app to the workspace + push + email to the page owner, once per page per day). Recovery is damped by a 5-min per-page Redis cooldown and an in-process single-flight. **Silent when it succeeds** — a re-minted token leaves no row change but `token_last_verified_at` |

...
- **Manual invoices (`invoices` + `invoice_documents`, migration 0186)**: the invoice register for every rail Stripe does not bill. Stripe emails its own VAT invoice for card subscriptions; a manual activation (`manualUpgrade`) sends **no email at all**, so a merchant paying by bank transfer, Sham Cash or through a reseller previously received nothing — the one invoice ever sent (InMedia, 2026-08-15) was built by hand outside the product. An admin issues one from the customer's Billing section: prefilled from the plan and current period but fully editable, because the amount collected is routinely not list price (InMedia's annual Pro was 790 USD, which no `plans` row contains). Numbering is `JW24-YYYY-NNN`, allocated **inside the same transaction** as the render and the insert under a per-series `pg_advisory_xact_lock`, so a failed render returns its number instead of tearing a gap in the series; Swedish bookkeeping requires the series to be unbroken, so an unwanted invoice is **voided, never deleted**. Everything printed is snapshotted onto the row — a repriced plan cannot rewrite an issued document. The bilingual AR/EN PDF is rendered by headless Chromium (`services/invoicePdf.ts`, `puppeteer-core` + the Alpine `chromium` package): Arabic needs bidi reordering and contextual shaping that the drawing-primitive PDF libraries leave to the caller, and the image already carried the Noto Arabic + Latin faces for the post-card compositor. The rendered bytes are archived in `invoice_documents` rather than re-rendered on demand, because the obligation is to reproduce the document **as sent**, not as a later Chromium would draw it. Sending reuses the merchant-email path unchanged (delivery, attachment sha256, `email_sends`, the `merchant_email_sent` audit row); the caller cannot supply the file — the server attaches the archived PDF by id, so "what we sent" and "what we stored" cannot diverge. Preview renders from unsaved input and allocates nothing. Issuer prints as the trade name (Jawab24) with the registered identity as footer small print, which is what keeps the document valid. Seller block and VAT wording are config (`INVOICE_*`), defaulting to the values already published on `/terms` §13. ⚠️ Does **not** record the payment — the `payments` ledger (PR #779) is still undeployed, and this register deliberately does not depend on it.

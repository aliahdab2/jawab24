...
- **A partner needs a normal Jawab24 account** — they sign up like any user (no pages, no subscription); the `partners` row is what turns that login into the portal. ⚠️ Binding matches `user_id` first, then **email OR phone** — and phone is not optional polish: Jawab24 has **no email login**, a Facebook signup carries whatever address is on the FB profile, and a phone-OTP signup (the primary identity) leaves `users.email` **NULL** (`authService.findOrCreateUserByPhone`). An email-only match therefore locks a phone-first reseller out permanently with no self-service fix. At least one anchor is required at creation; record whichever the partner will actually sign in with. The admin list shows `linked` so "has this rep actually got in yet?" is answerable. Regression coverage: `test/services/partnerBinding.test.ts` (asserts the phone appears in the built query, not merely that a mocked row came back).
...
- **الموزّع يحتاج حساباً عادياً في جواب24** يسجّله كأي مستخدم (بلا صفحات ولا اشتراك)، وسجلّ `partners` هو ما يحوّل هذا الحساب إلى بوابة موزّع. ⚠️ الربط يتم عبر `user_id` أولاً ثم **البريد أو الهاتف**؛ والهاتف ضروري وليس تحسيناً: لا يوجد تسجيل دخول بالبريد في المنصة، والتسجيل عبر الهاتف يترك `users.email` فارغاً تماماً — فالمطابقة بالبريد وحده تحجب الموزّع الذي يدخل بهاتفه عن البوابة نهائياً. يلزم إدخال أحدهما على الأقل عند الإنشاء

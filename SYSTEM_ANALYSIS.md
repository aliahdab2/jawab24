...
- **Schema**: `partners` table (name, `email`, `phone`, per-partner `commission_pct`, `user_id` portal binding, `is_active`) + `users.partner_id` (attribution, ON DELETE SET NULL) + `users.partner_note` (admin-authored follow-up note the assigned partner sees). Migration 0167.

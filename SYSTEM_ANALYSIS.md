...
- Instagram Graph API (comments + DMs) — Live (all permissions approved 2026-04-07). Two connect paths: **page-linked** (the Instagram Business Account attached to a connected Facebook Page — rides the Facebook page token on graph.facebook.com) and **Instagram-direct** (see below)
- Instagram-direct — "Instagram API with Instagram Login", **LIVE in production 2026-08-17** (credentials configured, frontend flag built in, first real account `@jawab24app` connected end to end with `subscribed_apps` reporting ok): a professional/creator account connects with its OWN credentials, no Facebook Page. The row carries `facebook_page_id NULL`, `access_token ''` and the encrypted `instagram_access_token` (migration 0169); calls go to **graph.instagram.com** with that token, paired by `services/instagramCredential.ts` so a token can never be sent to the wrong host. Endpoints are path-identical to the page-linked ones except the messages edge (`POST /{ig-id}/messages`, not `/me/messages`). Each account must ALSO install the app on itself (`POST /{ig-id}/subscribed_apps?subscribed_fields=messages,comments`) or no webhook ever arrives — done at connect, a failure is surfaced to the merchant rather than assumed, and the daily sweep re-issues the idempotent install for every live row so the state self-heals within 24h. 60-day token on a daily refresh sweep, and — like WhatsApp, and for the same reason — **excluded from Facebook page-token recovery**: there is no Facebook Page to re-mint from. ⏳ Meta App Review for `instagram_business_*` not yet submitted, so external merchants are gated on it: under Standard Access only an Instagram account holding a ROLE on app `774211662298446` (admin / developer / **Instagram Tester**) can complete the connect — a merchant who presses «إنستغرام فقط» today cannot. The submission's one blocker is the screencast (§`.planning/IG_LOGIN_APP_REVIEW.md`); ⛔ never press "Request again" on the stale 6 Mar rejection. Details: `.planning/codebase/INTEGRATIONS.md`
- WhatsApp Cloud API (DMs) — **LIVE (GA)**: Meta approved the app 2026-07-26; Embedded Signup connect + voice/media + WhatsApp-only cards (no Facebook page; also = multi-number, one card per number w/ own Business Info) + manual inbox replies + read receipts/typing indicators. **Coexistence (D-045)**: at connect time the merchant chooses whether the number stays live on their WhatsApp Business app (Meta's `whatsapp_business_app_onboarding`) or moves to Jawab24; a reconnect always re-uses the stored path. Meta enforces the precondition — a number not registered in the Business app is refused at the phone-number step rather than silently migrated. ⛔ Meta bars businesses AND recipients in Cuba, Iran, North Korea, **Syria** and three sanctioned Ukrainian regions from the WhatsApp Business Platform entirely — Syrian merchants can never connect and Syrian customers can never receive replies; Libya is unrestricted. UI: the Pages screen is now "Channels" (قنوات التواصل). **Plan-gated (Starter+, D-118)**: `plans.whatsapp_enabled` is true on starter/business/pro/scale, false on basic only (the trial rides on Starter, so trialing accounts are entitled) — enforced in `controllers/whatsapp.ts` on connect/connectNew/toggle-enable (403 `WHATSAPP_PLAN_REQUIRED`, `requiredPlan: 'starter'`; disconnect/disable never gated, no retroactive disable on downgrade). Basic sees an upgrade CTA instead of Connect; the pricing/scale/checkout pages list WhatsApp (crossed-out on Basic) via `isWhatsAppMarketable()` (= config env set AND canary flag off). **Readiness-gated (all channels)**: enabling auto-reply on a page with no groundable Business Info is refused with 409 `BUSINESS_INFO_REQUIRED` (`services/businessReadiness.ts`) — a WhatsApp-only card is created empty (no FB page to seed it) and the client used to enable auto-reply straight after connect, putting an ungrounded AI in front of customers; disable is never gated. **Connect from the NATIVE app (v2.0.20, owner-verified 2026-07-31)** mirrors the shipped Facebook page-connect flow exactly, and must keep doing so: the onboarding-path question is asked in-app, `POST /auth/whatsapp/start {nativeApp:true}` mints the dialog URL, and `Browser.open()` takes the tab **straight to facebook.com** — routing it through a jawab24.com page first fails silently on real devices (three variants died: page-side `location.assign` in a Custom Tab and in an intent-opened Chrome tab, and a server 302). The return is the `/auth/app-sync` App Link, delivered as a **page whose script navigates** — a 302 to an App Link is not intercepted by Android. Because the nonce cookie cannot cross the WebView/browser jar boundary, app-minted states are **single-use** instead (`lib/singleUseKey`). Full pattern: AI_INSTRUCTIONS Rule 17b
...
- **Sham Cash (offline rail, no API — LIVE behind env)**: merchants inside Syria cannot be charged by card (`utils/sanctions.ts` blocks SY before any Stripe call, and that block is UNCHANGED). Since D-110 `/checkout` shows them a **payment** screen instead of "payments are not available in your region": our wallet number with a copy button (the primary path — a merchant browsing on the phone that holds their wallet app cannot scan their own screen), an optional QR, and a form that submits the **transfer reference** plus an optional receipt image. The claim lands in `offline_payments` (`services/offlinePayments.ts`) as `pending_review`; support matches it against the wallet statement in `/admin/offline-payments`; **approving activates the plan for the claimed period in the same transaction** (`manualUpgrade` on the review's transaction — still the single grant choke point — with `granted_subscription_id`/`granted_at` stamped and a CHECK making approved ⇔ granted). Anti-replay is a PARTIAL unique index on `(rail, normalized reference)` across ALL accounts while the claim is not rejected (one reference names one real transfer; a refusal releases it); the normalizer is a whitelist (NFKC, letters+digits) so invisible characters cannot mint a second claim. A same-merchant resubmission is a replay (200 with the existing claim); a different account's is refused (409). Receipt bytes live in `offline_payment_receipts` in Postgres, **not** in the media bucket — `imageStorage` returns public URLs by design (`docs/OBJECT_STORAGE.md` §4) and a transfer receipt is a financial document; they are served only by the admin-authenticated route. Amount is USD, resolved server-side from the plan (no SYP rate machinery — the merchant transfers the equivalent). The wallet details are disclosed to any **authenticated** account, deliberately NOT gated on the request's IP country: VPN use is routine inside Syria, so an IP gate locked out the very merchants the rail is for (they resolved to Europe, saw the card form, and got a decline — the إن ميديا 2026-08-15 pattern). The number is a pay-TO address printed on the wallet's own QR card, not a secret. The same VPN case is why `/checkout` carries a «Paying from inside Syria?» link on the card form that switches to the Sham Cash panel regardless of geo. Reviewing a claim writes an `admin_audit_logs` row (`offline_payment_review`) in the same transaction as the status change, like every other admin mutation. Config: `SHAM_CASH_WALLET_NUMBER` / `_NAME` / `_QR_URL`; empty wallet number = rail OFF (`GET /payment/offline/config` → `{ enabled: false }`, `/health` → `services.shamCash.not_configured`) and the old WhatsApp notice returns; boot fails if the wallet is set without `ADMIN_EMAILS`. A daily digest alerts on claims pending over 48 h. Top-ups are NOT on this rail (a claim is filed against a plan). Hidden on iOS native (App Store 3.1.1), like every other payment surface.
- ~~Vonage SMS API~~ — **REMOVED 2026-09-03 (D-123).** The SMS rail is gone: customer notifications go out over WhatsApp only, team invites are email-only, and phone-OTP login has no transport at all (see Authentication Architecture)
...
- ~~Vonage SMS API~~ — **أُزيلت في 2026-09-03 (القرار D-123).** لم يبقَ مسار للرسائل النصية: إشعارات العملاء عبر واتساب فقط، ودعوات الفريق بالبريد الإلكتروني فقط، ولا وسيلة إرسال لرمز الدخول عبر الهاتف
...
| **greetingMessageMulti** | JSONB | seeded | `{ar: "...", en: "...", sourceLang: 'default'}` - welcome sent when a new customer taps "Get Started" / «بدء الاستخدام» (opener-only since 2026-08-17 — typed first messages go straight to the AI). ⚠️ Seeding is UNRELIABLE: only `workspace.ts:createWorkspace` seeds it, but real signups go through `auth.ts:createDefaultWorkspace`, which writes `NEW_SIGNUP_SETTINGS_SEED` and NO greeting — measured 2026-08-18, 56 of 83 workspaces carry an empty `{}` (every one created since 2026-05-14). Legacy rows were backfilled by migration `0095_backfill_default_greeting`. Because the stored value cannot be trusted, `getGreetingMessage` falls back at SEND time to `i18n.t('defaultGreeting', lang)` — the same strings the settings UI shows as its placeholder, so what merchants see in the empty field is what gets sent. **Sending is gated by `greetingMessageEnabled`** — the text alone no longer triggers a send. |
...
| **Merchant email (Resend)** | Implemented | The channel that reaches a merchant who is not in the dashboard and has not installed the app. Covers the subscription lifecycle (`trial_ending`, `trial_ended`, dunning: `payment_failed` / `service_suspended` / `payment_recovered`), page health (`auto_pause`, `page_reconnect`), and — since 2026-09-05 — the AI-usage thresholds (`ai_usage_warning` at 80% of the plan cap and when a top-up balance is nearly drained, `ai_usage_limit` when replies have actually stopped). Sent on the same crossing as the in-app card, once per threshold per billing period; the calm "now running on top-up" card has no email on purpose. `email_sends` audits every attempt |
| **E-commerce customer notifications** | Implemented | Abandoned cart, order confirmed/shipped/delivered, review request — BullMQ queue, per-store templates, Arabic/English auto-detected from phone prefix. **WhatsApp is the ONLY delivery channel (D-123, 2026-09-03): a Meta-approved UTILITY template sent from the merchant's own linked number.** It covers the four order/cart types; `review_request`/`digital_delivery` have no template and therefore no rail, so the card refuses to switch them on. The `channel` column and its per-type API field survive for a future rail, and accept `whatsapp` only. Abandoned cart fires on Salla + Zid (Zid `abandoned_cart.created`/`.completed` added 2026-08-25; `.completed` cancels the pending nudge); Shopify has no cart wiring |
...
| **Pages of a text-layer PDF that Vision revisits** | Decided **per page**: a page whose Arabic layer holds a table (`looksTabular` — a run of ≥3 row-shaped lines, shape only, no vocabulary) or a page with no usable layer of its own (a scanned page, a scanner-app watermark). Only those pages are rendered and sent to Vision — **with the page's text layer as the spelling authority** where one exists, so Vision only recovers layout — and the output is spliced back between the verbatim layer pages (`method: pdfjs+gpt-vision`). Vision denied (plan/quota) **or failed** → the merchant gets the text layer, never an error for a document already read. Before 2026-08-29 the heuristic counted any line with 3 tokens + a digit as a row, so a numbered manual was treated as *scanned* and its correct text discarded for gpt-4o-mini OCR (inverted meaning, invented words); the first fix still decided per document and sent all seven pages of that manual through Vision for one table on page 5 |
...
| ~~10~~ | ~~E-commerce customer notifications (order updates)~~ | ~~RESOLVED~~ | Shipped — SMS via Vonage, BullMQ worker, dedup by platformEventId, opt-in per template (is_enabled=false default). ⚠️ Scope corrected 2026-08-24: this covers `order_confirmed`/`order_shipped`/`order_delivered` on all three platforms. **Abandoned cart and review requests were over-claimed** — see gap 15. Delivery ran over Vonage SMS until the account proved unverifiable (#3002710) and the owner **dropped** that path on 2026-08-25; the WhatsApp channel (#957, 2026-08-26) replaced it, and the dead SMS rail was **removed** on 2026-09-03 (D-123). Measured before removal: all 12 template rows sat on the SMS channel, every one disabled, and `customer_notifications_log` was empty — the rail had never delivered anything |
| 15 | Three of the six customer-notification types don't do what their merchant toggle implies | Medium | Verified 2026-08-24 by tracing every `OrderEvent` producer into `customerNotificationService.schedule()`. **`digital_delivery` is ❌ NOT IMPLEMENTED** — template, API whitelist, analytics bucket and a merchant switch all exist; no code path can ever fire it. **`abandoned_cart` fires on Salla and — since #951, 2026-08-25 — Zid** (`controllers/salla.ts`; `controllers/zid.ts` handles `abandoned_cart.created`/`.completed`); Shopify never subscribes `checkouts/create`, so the "revenue recovered" analytics figure is structurally 0 for Shopify stores. **`review_request` sends a dangling link** — `orderNotificationScheduler.ts` hardcodes `review_url: ''` while the default body ends in `⭐ {review_url}`, and no endpoint can set it. Detail table in `backend/docs/SETTINGS.md`. ⚠️ The old note here calling `channel` "decorative" was stale from 2026-08-24: the WhatsApp rail shipped 2026-08-26 (#957) and reads it, and since D-123 `whatsapp` is the only value it accepts |
| 16 | Store policy sync (shipping / returns) is Shopify-only — **Zid shipping/payment closed 2026-08-30 (D-116)** | Medium | ✅ **Zid now writes `policiesSummary`** from its structured settings — enabled shipping methods (`GET /v1/managers/store/delivery-options`) and enabled payment methods with their fees (`GET /v1/managers/store/payment-methods`) — in `fullSync`, before the product sync's cache-invalidation tail. ❌ Zid **return/exchange policy still cannot be synced**: it is a dashboard "legal page" with no read API (the storefront Pages API serves custom pages only — verified live 2026-08-30 on the dev store), so it stays merchant-authored in Business Info (#965 nudges for it). ❌ Salla still writes nothing. Original finding: `syncPolicies` lives in `services/shopify.ts` and its only caller is Shopify's `fullSync`; Salla and Zid never write `policiesSummary`, so it is permanently NULL for those stores. Downstream, `storeAnswersPolicies()` is always false → `getEnrichedKnowledgeBase` drops the policy section and `invalidateCachesForStore` indexes no policy chunks. A Salla/Zid customer asking «كم يستغرق الشحن؟» / "what's your return policy?" is answered only from whatever the merchant typed into Business Info by hand — connecting the store does not cover it, though merchants reasonably assume it does. ⚠️ Scope narrowed 2026-08-25 (D-102): store **contact facts** (phone, WhatsApp, working hours, website) now sync into Business Info automatically for Salla and Zid (`storeFactsSync.ts`, provenance `store_sync`), and #953 nudges the merchant to author policies — so the remaining gap is **policies specifically**, not store info in general. The UI does not lie about it (`Page.storeAnswersPolicies` is derived from the same predicate). ⚠️ Even on Shopify the sync is thin: `POLICY_PREVIEW_LENGTH = 100` truncates each policy to 100 chars, and only shipping + refund policies are fetched |
...
| ~~10~~ | ~~إشعارات العملاء للتجارة الإلكترونية (سلة مهجورة، تحديثات الطلب، طلبات المراجعة)~~ | ~~تم الحل~~ | تم الشحن — BullMQ worker، مكافحة التكرار بـ platformEventId، اشتراك اختياري لكل قالب. جرى الإرسال عبر رسائل Vonage النصية حتى تعذّر توثيق الحساب (التذكرة #3002710)، فأوقف المالك ذلك المسار في 2026-08-25، وحلّت محله قناة واتساب (#957 في 2026-08-26)، ثم أُزيل المسار الميت في 2026-09-03 (القرار D-123). **واتساب هو القناة الوحيدة اليوم.** |
...
| Repeated send failures | `auto_pause` | present ⚠️ but possibly DEAD | set | merchant notified since 2026-08 **on Facebook/Instagram only** (in-app+push+email, `pageAutoPause.ts`; a WhatsApp-driven pause is silent by design — the copy is Facebook-specific); re-enable via the dashboard toggle (`pages.ts toggleAutoReply` clears the pause on off→on). ⚠️ A dead token (Graph 190/460, e.g. FB password change) reaching TEN failures still lands here — then the fix is reconnect FIRST, and a bare re-toggle just re-pauses. Since 2026-08-14 it should rarely get that far: see the row below |
| **Revoked credential, caught live** | `NULL` (auto-reply untouched) | **`''` (cleared)** | set | `services/pageTokenRecovery.ts`, since 2026-08-14. The FIRST Graph call that fails with a token-revoked code (190/458·459·460·463, 200/10) re-mints the page token from `/me/accounts`; if the merchant's own session is gone too, the page is marked disconnected and the merchant is told **which cause** it was — in-app to the workspace + push **once per page per day**, email to the page owner **once per OWNER per day** (the cause is per-user: one revoked session kills all their page tokens at once, so a page-scoped email dedup mails an agency owner once per dead page; both dedup claims are released whenever a working token is written back, so a re-revocation after a reconnect alerts again). Recovery is damped by a 5-min per-page Redis cooldown and an in-process single-flight, each with a per-process fallback so a Redis outage degrades the dedup instead of removing it. Facebook/Instagram only — WhatsApp's separate credential is excluded at `pageAutoPause.ownsFacebookCredential`, since Meta returns 190 for an expired WABA token too. ⚠️ **A successful recovery is silent to the MERCHANT, not to the row**: it writes `access_token` (the fresh one), `token_last_verified_at`, `disconnect_reason` (→ `NULL`) and `updated_at`. Do not read a changed token, a cleared disconnect reason or a fresh `updated_at` on this page as merchant action — check the logs (`[PageTokenRecovery] Page token re-minted`) before attributing it to a human |
...
**Rich product cards in DM** (shipped 2026-04-24): when a Messenger or Instagram DM triggers an ecommerce tool that surfaces a product reference (e.g. `check_inventory`) and the synced product has an image, the message pipeline sends a Generic Template carousel (image + price + "View product" button) as a follow-up to the text reply. Uses Meta's `template_type: "generic"` payload via shared `metaMessaging.ts`. Falls back to text-only when no image is available. Card-send failures are fire-and-forget (don't invalidate the text reply). Forward-compatible with WhatsApp Catalog (Phase 5 of `WHATSAPP_PLAN.md`) since the `MessagePlatformAdapter.sendProductCards` interface is platform-agnostic. **Since D-092 (2026-08-22) the product behind `check_inventory` is decided in code** (`reply/productResolver.ts`: trigram → reply embedding → `ambiguous` with candidates / `not_found` below a measured floor), stock is answered from the synced row with a by-id platform read only when the figure is risky, and the card is keyed on the result's `platformProductId` rather than re-matched by title. **Since 2026-08-23 the model names the products its reply presents (`respond.product_ids`, D-100) and those cards come first; a reply that links catalog products by URL cards those** (link cards — exact identity, carousel up to 10), ahead of the one-title mention rule that a catalog with repeated titles defeats; and no markdown reaches a customer — each platform adapter renders the canonical reply for its own channel right before sending (plain text, or WhatsApp's inline markup; see INTEGRATIONS.md «Per-channel reply rendering»). Sold-out products stay visible as "out of stock" in the catalog block, the KB chunks and the resolver instead of vanishing into "we don't sell that".
...
│  │   by email, with a copy-and-share link as fallback       │   │
...
- **Phone OTP — ❌ NOT IMPLEMENTED (no delivery transport; behind `PHONE_AUTH_ENABLED`, OFF)** — the code generates, stores and verifies codes (`otpService`, `otp_codes`, `/auth/phone/*`), but **nothing can carry one to a phone**. The SMS rail was removed with Vonage (D-123), and WhatsApp cannot substitute: a code goes to someone who has connected nothing, so it needs a **Jawab24-owned WABA** (none exists — every WhatsApp send uses a merchant's own number) plus an **AUTHENTICATION-category template** (`createMessageTemplate` submits UTILITY only). `otpService.sendOtp` therefore throws `OtpTransportUnavailableError` and the route answers **503 `otp_unavailable`** — deliberately loud, because the removed rail used to answer «sent» and deliver nothing. The store/verify half is kept because a WhatsApp OTP would reuse it; prerequisites in `.planning/WHATSAPP_PLAN.md`. **Syria stays permanently exempt whatever the transport** (D-045 — Meta bars it too): the prefix list is `SANCTIONED_DIAL_PREFIXES` / `isSanctionedPhone` in `@jawab24/shared`, checked in `requestOtp` before a code is minted.
...
- **Team invites are email-only, enforced server-side** (D-123). A phone contact is refused with 400 `email_required` in `workspaceInviteService.createInvite` before any row is written — until 2026-09-03 that rule was client-side only, so an API client could create an invite whose link could never arrive.
...
- **Schema**: `partners` table (name, `email`, `phone`, per-partner `commission_pct`, `user_id` portal binding, `is_active`) + `users.partner_id` (attribution, ON DELETE SET NULL) + `users.partner_note` (admin-authored follow-up note the assigned partner sees). Migration 0168.
- **A partner needs a normal Jawab24 account** — they sign up like any user (no pages, no subscription); the `partners` row is what turns that login into the portal. ⚠️ **Binding matches `user_id` first, then the PHONE — never the email.** `users.phone` is unique and can only be set by proving possession via OTP (`authController.linkPhone`), which is what an auto-bind anchor has to be. `users.email` is neither: `PATCH /auth/profile` lets **any authenticated user set an arbitrary address with no verification**, and the column is not unique. Auto-binding on it let any merchant who knew a rep's address claim that rep's portal before the rep first opened it — taking permanent possession of the row, reading every attributed merchant's name/phone/note, and locking the real rep out (found in review of PR #764, fixed before merge). Record the partner's phone and they bind themselves; a partner with **no usable phone anchor must be linked by an admin** (`PUT /admin/partners/:id { userId }`, exposed as the **Link account** picker in `PartnerManagerModal`). ⚠️ "Has a phone on file" is NOT the same as "can auto-bind", and reading them as equivalent stranded the first real reseller: the anchor needs a **user account** carrying that number, verified by OTP. A rep who signs up with **Facebook** has `users.phone = NULL`, and a rep in a **sanctions-blocked** country (Syria, `+963` — listed in `SANCTIONED_DIAL_PREFIXES`, and blocked on every transport, not just the retired SMS rail: Meta bars WhatsApp there too, D-045) can never verify one. ⚠️ Since D-123 **nobody** can verify a phone — there is no OTP transport at all — so auto-bind is unavailable to every rep until a WhatsApp OTP exists; the admin **Link account** picker is the only path today. Both show a phone in the admin list and can never bind on their own. So the manual-link warning fires on **every** unlinked partner, not only the phone-less ones — the old `!linked && !phone` predicate hid it from exactly the reps who needed it. Likewise `linked` answers only "is a login bound", **never** "has this rep signed in": it was labelled "Signed in" / «سجّل دخوله», which told the operator to wait for something that could not happen. The bind writes an audit row (`partner_portal_bound`) and a concurrent claim loses at the database (`WHERE user_id IS NULL`, result checked). Regression coverage: `test/services/partnerBinding.test.ts` (asserts the phone appears in the built query and the **email never does**, not merely that a mocked row came back).
- **Admin** (`/admin/customers`): per-merchant reseller assignment dropdown + note editor + a reseller registry modal (`PartnerManagerModal`: create, edit contact/commission, **link** an unlinked reseller to an account via a search picker pre-filled with their own contact details, **unlink** a wrong portal binding, **deactivate**); the list also shows a country hint derived from the E.164 phone prefix (libphonenumber, e.g. +963 → Syria). Endpoints: `GET/POST /admin/partners`, `PUT /admin/partners/:partnerId`, `PUT /admin/users/:userId/partner` (all `requireAdmin`, audit-logged as `partner_created` / `partner_updated` / `partner_assigned`). Unlink (`userId: null`) is the recovery path when a binding lands on the wrong account, and `isActive: false` cuts a reseller's access to merchant data on their next request — without both, either situation needs a hand-written UPDATE against production.
- **Partner portal** (`/partner`, `GET /partner/overview`): read-only page for the partner's own login. Binding is lazy — admin records the partner's phone; on first visit the portal matches `users.phone` and persists `user_id`. Shows ONLY the partner's attributed merchants (accounts with ≥1 connected page, newest first, cap 500): name, phone, page names, plan, derived status (stale `trialing` rows past `trial_ends_at` render as trial-expired without mutating), trial/period end, signup, last activity, admin note. Non-partners get 403 → redirected.
- **How the reseller finds it**: a **Partner Portal entry in the ACCOUNT nav group**, shown only when the session carries `isPartner` — resolved server-side by `isPartnerUser` (`services/partnerAccess`) on every login path and on `/auth/me`, and re-applied from `/auth/me` by `lib/sessionSync.syncSessionState` on each authenticated page load, **on web and inside the app alike**, so a rep registered *after* their device signed in picks it up without a re-login. ⚠️ That reconciliation must stay platform-agnostic: gating it on web (as the first cut did) freezes the entry at its login-time value on the ONE surface that cannot reach `/partner` by URL, against a 60-day native token. The demo login is the one path that never resolves the flag — the demo user is shared, so a matching partner row would expose a reseller's entry to every visitor; `/auth/me` corrects it for real partners on the first page load. The flag decides **visibility only**; `/partner/*` re-resolves the partners table on every call. ⚠️ `isPartnerUser` is deliberately NOT `resolvePartnerForUser` — the latter *claims* an unbound row, which is correct on the portal's own endpoints and catastrophic on the login path, where it would bind a rep's row to whoever logged in first. Both nevertheless match on the **same two anchor expressions** (`partnerBoundToUser` / `partnerClaimableByPhone`, exported from the low-dependency `partnerAccess` module and imported by `partnerPortal`, never the reverse) — a nav entry matched on looser terms than access grants would advertise a portal that 403s. The portal itself carries a **back-to-dashboard link**: it is a standalone layout with no sidebar, and iOS configures no back gesture, so without it the app is a one-way door. Until this shipped the portal had **no link anywhere**: reachable only by typing the URL, which meant unreachable inside the mobile app (no address bar, and the App Link filter covers only `/auth/app-sync`). Regression coverage: `frontend/src/components/layout/__tests__/resolveNavKey.test.tsx` (gating, incl. native), `frontend/src/lib/__tests__/sessionSync.test.ts` (reconciliation, pinned with native mocked true), `backend/test/services/partnerAccess.test.ts` (`isPartnerUser` writes nothing) and `backend/test/services/partnerBinding.test.ts` (claim path uses the shared anchors).
- **Partner merchant detail** (`/partner/merchant?merchantId=`, `GET /partner/merchants/:userId`): drill-down with the support-console sections — overview, connected pages (+ per-page connection/auto-reply state and Business Info health), current-period usage, lead counts, settings, top-up balance, team. Reuses `adminUsersService.getUserDetail` behind two guards:
  - **Ownership gate**: the merchant must carry the *calling* partner's `partner_id`, resolved server-side from the session — never from a client parameter. A merchant belonging to another partner answers **404, not 403**, so the status code cannot be used to enumerate ids.
  - **Allowlist projection** (`toPartnerMerchantDetail`), never a denylist: `getUserDetail` keeps growing, and spreading it would hand resellers every future field. Omitted: merchant + workspace-owner `email`, `facebookId`, `aiModel`, and `health` (its flag catalog is admin-namespace i18n). Costs never appear — AI spend lives behind admin `/ai-cost`, which partner routes do not expose.
  - **Configured-or-not, never content** (`toPartnerSettings`): toggles pass through because "Smart Replies off" is the top cause of a failed trial; merchant-authored free text (brand voice, greeting, away message) collapses to `hasBrandVoice` / `hasGreetingMessage` / `hasAwayMessage` booleans **server-side**, so the wording never reaches the reseller's browser. Business Info is the upstream summary (length, chunk counts, unresolved gaps) — never the text. `settings.source === 'legacy-fallback'` renders a warning, because unconfirmed values shown as fact are how a rep tells a merchant their setup is fine while replies are off (the D-026/#608 failure mode).
  - Regression coverage: `test/services/partnerMerchantDetail.test.ts` (gate short-circuits before any data fetch; deep-scan assertions that persona/greeting text and both emails are absent) and `test/routes/partner.test.ts` (401/403/404/200 + "scopes to the calling partner, never a client-supplied id").
...
- **المخطط**: جدول `partners` (الاسم، البريد، الهاتف، نسبة عمولة لكل موزّع، ربط الحساب) + `users.partner_id` (الإسناد) + `users.partner_note` (ملاحظة متابعة يكتبها المدير ويراها الموزّع)
- **الموزّع يحتاج حساباً عادياً في جواب24** يسجّله كأي مستخدم (بلا صفحات ولا اشتراك)، وسجلّ `partners` هو ما يحوّل هذا الحساب إلى بوابة موزّع. ⚠️ الربط يتم عبر `user_id` أولاً ثم **الهاتف وحده، لا البريد الإلكتروني**: رقم الهاتف فريد ولا يُسجَّل إلا بإثبات الملكية عبر رمز التحقق، أما البريد الإلكتروني فيستطيع أي مستخدم مسجَّل تعيينه لنفسه بأي قيمة دون أي تحقق، وليس فريداً — لذا كان الربط بالبريد يتيح لأي تاجر يعرف بريد المندوب أن يستولي على بوابته نهائياً ويطّلع على أسماء تجّاره وأرقامهم (اكتُشف في مراجعة PR #764 وعُولج قبل الدمج). سجّل هاتف الموزّع ليرتبط بنفسه؛ والموزّع الذي لا هاتف له يلزم ربط حسابه يدوياً من لوحة الإدارة
- **الإدارة** (`/admin/customers`): إسناد الموزّع لكل تاجر + محرر الملاحظة + سجلّ الموزّعين (إضافة، تعديل بيانات التواصل والعمولة، **فكّ ربط** حساب ارتبط بالخطأ، **إيقاف** الموزّع)، مع إظهار بلد التاجر المشتق من مفتاح الهاتف (+963 → سوريا). فكّ الربط هو طريق التعافي الوحيد، والإيقاف يقطع وصول الموزّع إلى بيانات تجّاره عند أول طلب تالٍ
- **بوابة الموزّع** (`/partner`): صفحة قراءة فقط تعرض للموزّع تجّاره فقط (الحسابات التي لديها صفحة مرتبطة، الأحدث أولاً): الاسم، الهاتف، الصفحات، الخطة، الحالة، تواريخ التجربة والاشتراك، آخر نشاط، ملاحظة المدير
- **صفحة تفاصيل التاجر** (`/partner/merchant?merchantId=`): نظرة عامة، الصفحات المربوطة وحالتها، استخدام الفترة، أعداد العملاء المحتملين، الإعدادات، رصيد الردود الإضافية، الفريق. القاعدة الحاكمة: **الموزّع يرى ما إذا كان الإعداد مضبوطاً، لا مضمونه** — فالمفاتيح تظهر مُفعّل/متوقف، بينما نصوص التاجر (الشخصية، الترحيب، الرد خارج الدوام) تتحول إلى قيم منطقية في الخادم فلا تصل إلى متصفحه أبداً، ومعلومات النشاط التجاري تظهر كحجم وعدد أسئلة دون إجابة لا كنص. ولا تظهر إطلاقاً: التكاليف، البريد الإلكتروني، المحادثات. أي تاجر غير مُسند إلى الموزّع يُجاب بـ 404 وليس 403 حتى لا يُستدل على وجود الحساب
...
`{ count, latestName, latestAt, oldestAt, byPage }`):
...
2. **Nav badge** (sidebar + mobile "More") — reads the server count via
   ...
   **status changes**, not when the merchant merely visits `/leads`. Because it
   outlives the visit it must stay resolvable: it links to `/leads?status=new`
   and lands on a page that actually has waiting leads (from `byPage`). See
   "Resolving the badge" in [docs/leads.md](docs/leads.md).
...
`/leads` page — page selector (entries with waiting leads are labelled with their count), status filter tabs (All/New/Contacted/Converted) **mirrored in `?status=`** so a deep link or reload reproduces the view, dynamic table columns from `extractedData.fields`, CSV export (incl. sub-stage + custom-field columns), real-time SSE updates via `lead:captured` event, server-backed new-leads badge in sidebar (see "Visibility of the standing queue" above — visiting the page no longer clears it). Detail panel ordered by merchant workflow: AI intent summary + extracted details first, then contact actions, status + sub-stage picker, custom data fields. "Customize leads" opens `StageCustomizerModal`.

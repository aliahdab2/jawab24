...
- **How the reseller finds it**: a **Partner Portal entry in the ACCOUNT nav group**, shown only when the session carries `isPartner` — resolved server-side by `partnerPortalService.isPartnerUser` on every login path and on `/auth/me`, and re-applied from `/auth/me` on each authenticated page load so a rep registered *after* their device signed in picks it up without a re-login. The flag decides **visibility only**; `/partner/*` re-resolves the partners table on every call. ⚠️ `isPartnerUser` is deliberately NOT `resolvePartnerForUser` — the latter *claims* an unbound row, which is correct on the portal's own endpoints and catastrophic on the login path, where it would bind a rep's row to whoever logged in first. Until this shipped the portal had **no link anywhere**: reachable only by typing the URL, which meant unreachable inside the mobile app (no address bar, and the App Link filter covers only `/auth/app-sync`). Regression coverage: `frontend/src/components/layout/__tests__/resolveNavKey.test.tsx` (gating, incl. native) and `backend/test/services/partnerBinding.test.ts` (`isPartnerUser` writes nothing).

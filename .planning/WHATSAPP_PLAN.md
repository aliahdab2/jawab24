...
### Phase 2: Frontend — WhatsApp Connection UI
- [ ] Facebook Embedded Signup flow for WhatsApp Business Account
- [ ] "Connect WhatsApp" button on pages.tsx
- [ ] WhatsApp phone number display + auto-reply toggle
- [ ] Translation keys for WhatsApp UI strings

### Phase 3: Template Messages (24h window)
...
### Business Model
ManyChat model — merchant connects their own WhatsApp Business Account via Facebook Embedded Signup. Meta bills the merchant directly for per-message costs. Jawab24 is the automation middleware.

### How it works
1. Merchant connects WhatsApp via Embedded Signup → gets `phone_number_id` + access token
2. Meta sends webhooks to `/webhook` with `object: "whatsapp_business_account"`
3. Webhook handler extracts text message, enqueues as `whatsapp_message` job
4. Worker delegates to `whatsappReplyService` → `messageProcessor` (same AI pipeline)
5. Reply sent via WhatsApp Cloud API (`POST /{phone_number_id}/messages`)

...
5. **Services** (`src/services/`):
   ...
   - Subdomain services:
     ...
     - `reply/` — Reply generation pipeline (context, formatting, quality checks)
       - `commentProcessor.ts` — unified comment pipeline: Post Reply rule check (keyword match or any-comment mode) → content-free gate (D-111, inside the generator: an uninvited «.»/«❤️» is resolved `uninvited_symbol` before any model call) → AI generation → send → lead extraction (fire-and-forget). Any-comment mode runs — inside the per-comment idempotency check + reply lock — the shared skip rules + a no-AI complaint keyword-guard + the handoff-pause gate + an invisible per-post/24h cap before sending (see `postReplyRule.ts`, `protection/post-reply-cap.ts`, D-013)
       - `commentCta.ts` — pure half of the D-111 gate: comment shape (`dot|digits|heart|emoji|text`, modifiers stripped, hearts from `spamPatterns.HEART_CODEPOINTS`), concept matching against the post's stored CTA verdict, and `decideContentFreeGate` (`shadow` = count only / `enforce` = skip). Text comments never enter it. Shared by production and the playground/eval (Rule 19).
       - `../contentCtaClassifier.ts` — model half: one pinned `gpt-4.1-mini` structured-output call per post, on the first content-free comment (single-flight across concurrent workers), persisted in `content_cta_classifications` by caption hash with per-post skip tallies; metered as `post_cta_classification`, never against the reply quota. Only model-authored verdicts persist. Env: `COMMENT_CTA_GATE_MODE`, `COMMENT_CTA_CONFIDENCE_THRESHOLD`, `COMMENT_CTA_CLASSIFIER_ENABLED`.
   ...
   - Backend i18n: `utils/i18n.ts` — centralized customer-facing strings (nudges, fallbacks, placeholders), plus system-authored model INPUT that needs per-locale variants (`contentFreeCtaQuestion`, D-107). Add new languages by extending the `Locale` type — `ar: ar satisfies typeof en` makes a half-added locale a compile error (`TS2740`), verified by mutation. ⚠️ "extend the `Locale` type" is optimistic: registering a third locale also produces ~8 `tsc` errors where `'ar' | 'en'` is hardcoded in unrelated signatures (`controllers/paymentWebhookHandlers.ts`, `services/dunningNotices.ts`). `tsc` finds every site, so it is mechanical — but it is a typed refactor, not a drop-in JSON file.
   - Reply-language ladder: `utils/replyLanguage.ts` — the ONE answer to "what language do we write in when the customer's text carries no language signal?". Holds `resolveFallbackLanguage` (canned away/quota/price templates: text → post → KB → merchant default → en, narrowed to `'ar' | 'en'`) and `resolveAuthoredCtaLanguage` (the synthetic content-free-CTA question: merchant default FIRST, returning a language CODE so `sv`/`fr`/`tr` survive — D-107). It lives in `utils/` and not in `services/reply/generator.ts`, its original home, because `generator.ts` imports `commentPreprocess.ts` — the other consumer — so keeping it there and importing it back would be circular, and pulling `generator` (DB pool, config, OpenAI client) into a pure text module would invert the layering. `generator.ts` re-exports `resolveFallbackLanguage` so its six call sites and the suites that mock them are untouched.

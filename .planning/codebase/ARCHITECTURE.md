...
5. **Services** (`src/services/`):
   ...
   - Subdomain services:
     ...
     - `reply/` — Reply generation pipeline (context, formatting, quality checks)
       ...
       - `productCardBuilder.ts` — builds `ProductCard[]` for the Messenger/Instagram Generic Template carousel from two sources: ecommerce tool results (e.g. `check_inventory`), and — when no tool produced one — the single in-stock catalog product the reply text names (mention cards; local Arabic/case-folded match, exactly-one rule). Also owns the per-customer 24h card cooldown (`filterRecentlySentCards`). Requires a synced image + handle from `ecommerceProducts`; degrades to text-only otherwise and never throws.

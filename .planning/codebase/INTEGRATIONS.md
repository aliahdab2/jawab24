...
- **Connection Flow (implemented — Embedded Signup)**:
  ...
  - **Connect entry points** (one rule: global → new card, card → attach here): the Channels header has a single "Connect channel"/«ربط قناة» button opening `ChannelPickerModal` (Facebook Page vs WhatsApp Number; WhatsApp option env-gated — without config the button collapses to the FB dialog directly). Attaching WhatsApp to an existing page's Business Info stays contextual via that card's WhatsApp row

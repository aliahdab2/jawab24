...
- **No markdown reaches a customer** (since 2026-08-23): the backend's single AI dispatch point (`reply/generator.ts:dispatchAiReply`, shared by DMs, comments and the playground/eval) rewrites `[label](url)` → `label: url` and `![alt](url)` → `url` via `@jawab24/shared` `stripMarkdownLinks`. Messenger, Instagram and WhatsApp render markdown literally; the model only began emitting it once real storefront links entered its catalog block (D-097) — 4 of 82 replies on the Salla page that afternoon, 0 in the other 40k of the month. The eval harness carries a matching invariant (`invariant:no-markdown-links`).

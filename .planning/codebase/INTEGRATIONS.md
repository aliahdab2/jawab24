...
**Legacy (v2.0.19 clients only):** public `GET /auth/whatsapp/app-start?code=…` consumes a single-use browser-handoff code, signs the browser in, and serves a handoff page whose anchor carries the merchant to Meta. Kept only until v2.0.19 falls out of the install base; it shares its gate/mint core with `start` via `prepareStartUrls`.
  ...
  - **Subscription-status gate at connect (D-118 follow-up)**: making WhatsApp a Starter feature removed the side effect where the Starter-only plan flag *also* kept expired-trial accounts out, so an inactive subscription on a WhatsApp-included plan could reach Meta ES — which migrates the number off the merchant's phone before enable is ever reached. `checkWhatsAppSubscriptionStatus` (in `controllers/whatsapp.ts`, keyed on the workspace OWNER, delegating to `subscriptionsService.checkSubscriptionStatus`) now runs at every connect entry point — `connect`, `connectNew`, the redirect `prepareStartUrls` and the callback `reverifyGates` — after the plan gate and before any Meta call → 402 `SUBSCRIPTION_INACTIVE` (popup, via `pageGateError`) / `?whatsappError=WHATSAPP_SUBSCRIPTION_INACTIVE` (redirect). Enable stays gated by `canEnablePage` (status + slots). The frontend `whatsappEntitled` mirrors it: `plan.whatsappEnabled && usage.subscription.autoReply.allowed !== false`, so an expired-trial owner never sees Connect

...
- **Rich Product Cards**: The reply pipeline sends a follow-up Generic Template carousel with the product image, price, and a `View product` button. Two triggers, in `backend/src/services/ecommerceToolLoop.ts`:
  ...
  2. **Mention cards** (since 2026-08-22) — the reply text names **exactly one** catalog product that is in stock (`totalInventory` null = unlimited, or > 0) and has an image and a handle. This covers the common small-catalog path: the catalog is inlined in the prompt whole, so the model answers directly and never calls a tool — before this, those replies carried a bare URL and no card. Matching is a local, Arabic-folded, case-folded substring check (no model call — Rule 17); zero or several matches → no card, because a wrong card is worse than none. Consulted only when no tool-result card exists. The scan is ordered by product id and bounded at 1000 rows, and a catalog **larger** than that window yields no card at all: the exactly-one rule is only sound over the whole active catalog, so on a truncated view a reply naming two products could leave exactly one inside the window and card it — the very case the rule exists to suppress. Bigger catalogs are not inlined in the prompt anyway, so the model reaches for a tool there and tool-result cards still fire.

  Either way, a card for the same product goes to the same customer at most **once per 24h** (`filterRecentlySentCards`, Redis `SET NX EX`, fails open) so a conversation that keeps naming one product does not re-send its card every turn. Payload building (truncation, Meta limits, messaging_type) lives in `backend/src/services/metaMessaging.ts` and is shared by Messenger and Instagram. The card build/lookup lives in `backend/src/services/reply/productCardBuilder.ts`. Card send failures are logged but don't invalidate the text reply already delivered.

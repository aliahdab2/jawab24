...
- **Rich Product Cards**: The reply pipeline sends a follow-up Generic Template carousel with the product image, price, and a `View product` button. Two triggers, in `backend/src/services/ecommerceToolLoop.ts`:
  ...
  2. **Mention cards** (since 2026-08-22) — the reply text names **exactly one** catalog product that is sellable (`totalInventory` null = untracked/unlimited; anything <= 0 is not) and has an image and a handle. This covers the common small-catalog path: the catalog is inlined in the prompt whole, so the model answers directly and never calls a tool — before this, those replies carried a bare URL and no card. Matching is a local, Arabic-folded, case-folded substring check (no model call — Rule 17); zero or several matches → no card, because a wrong card is worse than none. Consulted only when no tool-result card exists.

...
- **Order lookup without the order number** (D-101, 2026-08-23): `find_order_by_phone` takes the customer's phone **and** the name on the order in one call and returns the newest order where BOTH match. The platform search is only a candidate source — Zid `orders?search_term=` also matches names/codes, Salla has no order phone filter (`customers?keyword=` → `orders?customer_id=`) — so `handlePhoneLookup` re-compares with the same `phonesMatch`/`namesMatch` as `verify_and_get_*`. Unknown phone and wrong name return the same `order_not_found`. Never cached; fan-out bounded; counters `metrics:ecom:phone_lookup:*`.
- **The tool path's final reply is a strict `respond` function call** (D-099, 2026-08-23): on store-linked pages both OpenAI requests carry the data tools plus `respond` — whose parameters are the shared reply schema — with `tool_choice: 'required'`. The envelope is a function argument generated under the strict grammar, never message text; a data tool called beside `respond` wins. Measured 31/31 replies via `respond` at temperature 0, Cat 81 3+1 → 4/4; `check_inventory` is now called on stock questions as the prompt asks (1 → 10 over 15 cases, ~+0.7 s). Logged per reply as `tool_path_final` `via: respond|content`. Touches only `TOOL_PROMPT_ADDITION` (store pages) — no `PROMPT_VERSION` bump.
- **Per-channel reply rendering** (since 2026-08-23): the model writes ONE canonical, markdown-capable reply; each platform adapter owns `renderReply(text)` and turns it into what ITS channel displays (`@jawab24/shared` `renderReplyForChannel`): Messenger, Instagram and the comment adapters → plain text (links become `label: url`, `**bold**`/`__italic__`/`~~strike~~`/headings unwrapped), WhatsApp → its own inline markup (`*bold*`, `_italic_`, `~strike~`, a heading as a bold line). Both pipelines call it immediately before `sendReply` — before the length cap, so the cap measures what is sent — and persist the rendered text, so the stored row is what the customer saw. The playground renders for the page's channels too (`playgroundRenderTarget`: a WhatsApp-only row → WhatsApp markup, anything else → plain), so the test surface and the eval see customer-visible text. No channel renders markdown; the model only began emitting it once real storefront links entered its catalog block (D-097) — `![فستان](<product page>)` on the Salla page, 4 of 82 replies that afternoon, 0 in the other 40k of the month. The eval harness carries the invariant `invariant:no-markdown-links`.
- **Rich Product Cards**: The reply pipeline sends a follow-up Generic Template carousel with the product image, price, and a `View product` button. Sources in priority order, in `backend/src/services/ecommerceToolLoop.ts`:
  0. **Model-named ids** (D-100, since 2026-08-23) — `respond.product_ids`: the model lists the platform IDs of the products its reply presents (the `(ID: …)` printed in the catalog block and RAG chunks). Resolved by `platform_product_id` within the store, model's order, unknown ids counted and skipped (`metrics:product_card:model:*`). When the model names products, nothing is inferred from prose; the triggers below are the fallback.
  1. **Tool-result cards** — `check_inventory` answered about a product. Since D-092 the card is keyed on the result's own `platformProductId` (image from the result, else the row by key) — never re-resolved by title, which was a second matcher that could card a product the answer never named. Outcomes: `metrics:product_card:tool:{fired,no_identity,no_url,no_image,error}`; an `ambiguous_product` answer never cards a candidate.
  2. **Link cards** (since 2026-08-23) — the reply text carries the canonical storefront URL of one or more catalog products (`productUrlFor` — the same URL the catalog block gives the model since D-097). A linked product is an exact identity, so every sellable, imaged one gets a card, in reply order, up to the Generic Template's 10-card carousel limit. This is what the title rule below cannot do on a catalog with repeated titles: the Salla demo store (9 × «فستان», 5 × «تنورة») answered every skirt question `several_matches` while the reply's own link named one specific row. The reply's URLs are percent-decoded before matching (the model emits both forms of an Arabic path). Outcomes at `metrics:product_card:link:{fired,out_of_stock,no_image,capped}`. Consulted after tool-result cards and before mention cards; when every linked product is unsellable or image-less, the title rule still runs.
  3. **Mention cards** (since 2026-08-22) — the reply text names **exactly one** catalog product that is sellable (`totalInventory` null = untracked/unlimited; anything <= 0 is not) and has an image and a handle. This covers the common small-catalog path: the catalog is inlined in the prompt whole, so the model answers directly and never calls a tool — before this, those replies carried a bare URL and no card. Matching is a local, Arabic-folded, case-folded substring check (no model call — Rule 17); zero or several matches → no card, because a wrong card is worse than none. Consulted only when no tool-result card exists.

     Two properties of the "exactly one" rule are load-bearing and easy to break. It is decided over the **whole** active catalog: the scan is ordered by id and reads one row past its cap, and a catalog larger than the cap returns no card at all rather than deciding on a slice (a second match hiding outside the slice is exactly how "several → none" stops holding). And a title only counts as named when it is not glued to the end of a longer Latin word; single-token titles shorter than 6 characters never match, so a store that lists «شحن» as a product does not card it on «الشحن مجاني». Outcomes are counted at `metrics:product_card:mention:{fired,no_match,several_matches,out_of_stock,no_image_or_handle,scan_capped,no_store,error}`. The tool path has its own counter since 2026-08-22: every `executeToolCall` increments `metrics:ecom:tool:{tool_name}:{success|cached|<error code>}` exactly once — the order tools had never recorded an invocation in production, and this is the signal that says whether the first real `lookup_order` ran, and whether it was refused by our sanitizer (`invalid_order_number`) or by the platform (`order_not_found`).

  Either way, a card for the same product goes to the same customer at most **once per 24h** — and the window is opened by `markCardsSent` only **after** the platform accepts the send, never by the `filterRecentlySentCards` read in front of it. A card built and then swallowed by that window is counted at `metrics:product_card:cooldown:suppressed` (since D-092 — before, the builder's `fired` could exceed the cards actually delivered with nothing to explain the difference). Claiming it up front meant a card that failed to deliver silenced itself for 24h, inverting the fail-open contract. Card subtitles and the button label are `t(key, lang)` in the reply's own language (Rule 13b) — the tool path states availability from its LIVE `check_inventory` result, the mention path from the last product sync, so the latter uses the same in-stock/low-stock vocabulary as the catalog block the model answered from. Payload building (truncation, Meta limits, messaging_type) lives in `backend/src/services/metaMessaging.ts` and is shared by Messenger and Instagram. The card build/lookup lives in `backend/src/services/reply/productCardBuilder.ts`. Card send failures are logged but don't invalidate the text reply already delivered.
...
- **Status**: LIVE (GA) — channel shipped #392 + follow-ups (#418/#420/#423/#424); Meta App Review cleared 2026-07-26, GA per `docs/WHATSAPP_LAUNCH_RUNBOOK.md` Phase 5. **Packaging: included from the Starter plan up** (`whatsappEnabled` on starter/business/pro/scale; Basic excluded — D-118). Enforced server-side (`controllers/whatsapp.ts`): a **plan** gate (403 `WHATSAPP_PLAN_REQUIRED`) at connect/toggle, plus — since the D-118 follow-up — a **subscription-status** gate at connect (`checkWhatsAppSubscriptionStatus`, 402 `SUBSCRIPTION_INACTIVE` popup / `?whatsappError=WHATSAPP_SUBSCRIPTION_INACTIVE` redirect) so an expired-trial Starter cannot reach Meta Embedded Signup; the enable branch already status-gates via `canEnablePage`
...
**Legacy (v2.0.19 clients only):** public `GET /auth/whatsapp/app-start?code=…` consumes a single-use browser-handoff code, signs the browser in, and serves a handoff page whose anchor carries the merchant to Meta. Kept only until v2.0.19 falls out of the install base; it shares its gate/mint core with `start` via `prepareStartUrls`.
  ...
  - **Plan gate (Starter+)**: WhatsApp is a plan entitlement (`plans.whatsapp_enabled` — true on starter/business/pro/scale, false on basic; trial rides on Starter → included, D-118). Enforced in `controllers/whatsapp.ts` (`hasWhatsAppPlanAccess`, keyed on the workspace OWNER's subscription like `canEnablePage`) on `connect`, `connectNew` and the toggle's enable branch → 403 `{ code: 'WHATSAPP_PLAN_REQUIRED', requiredPlan: 'starter' }`, after the canary allowlist and before any Meta call. Disconnect/disable are never gated; no retroactive disable on downgrade (matches the maxPages model). Frontend reads the entitlement via the shared `useSubscriptionUsage` hook (`['subscription-usage']` key, SSE-invalidated): non-entitled owners get an `UpgradeCTA` instead of Connect, the channel picker collapses to the FB dialog, and the dashboard nudge banner is suppressed
  - **Connect entitlement gate (D-121)**: ONE chain, `checkWhatsAppConnectEntitlement(workspaceOwnerId)` in `controllers/whatsapp.ts`, called by every connect entry point and nothing else — `connect`, `connectNew`, the redirect `prepareStartUrls`, the callback `reverifyGates`. Order is **plan → marketplace → status**: permanent blocks before transient ones, so a Zid-blocked account (D-117) is never told to renew for something renewal cannot unlock. ONE `getUserSubscription` read serves the plan check and the status check. Refusals: 403 `WHATSAPP_PLAN_REQUIRED` / 403 `WHATSAPP_UNAVAILABLE_FOR_MARKETPLACE` / 402 `WHATSAPP_SUBSCRIPTION_INACTIVE` — the SAME code on both transports (JSON body on the popup + `/start` legs, `?whatsappError=<code>` on the redirect return), bodies static so the service's internal English `reason` never reaches a client. Why connect status-gates at all when a page slot is enforced at ENABLE (`canEnablePage`): finishing a connect runs `completeWhatsAppSignup`, whose `registerPhoneNumber` call takes the number OFF the merchant's phone — refusing at enable would be after that. Before D-118 the Starter-only plan flag doubled as this gate. Frontend mirrors it as TWO booleans (`whatsappPlanIncluded`, `whatsappSubscriptionActive`) so the CTA names the action that unblocks them (renew vs upgrade), guarded at `requestConnectWhatsApp` — the funnel every entry reaches, including the reconnect banner and the `?connectWhatsApp=true` handoff, neither of which the render-level hiding covers. One client-side code→message map: `lib/whatsappConnectErrors.ts`
  - **Readiness gate (all channels)**: enabling auto-reply on a page the AI cannot ground a single answer from is refused → 409 `{ code: 'BUSINESS_INFO_REQUIRED' }`. `services/businessReadiness.ts` `businessInfoGate()` (returns the refusal or null, shaped like `pageGateError` so every toggle reads the same; a null/absent page skips the gate so the handler's own 404 owns it) passes the page if ANY source reaches the prompt — Business Info text, authoritative structured facts (BUSINESS_INFO block: merchant-confirmed, kb_extract, or D-102 store-synced `store_sync` facts — so a store-synced phone honestly widens this gate), descriptive FB fields (narrative block), a live store whose `getStoreContextForAI` returns content, or ≥1 catalog item — and each is tested by calling that source's own formatter rather than re-deriving "is this set?". Wired into all three toggles (`controllers/whatsapp.ts` after the plan gate, `controllers/pages.ts` and `controllers/instagram.ts` before the billing gates); DISABLE is never gated. Exists because a WhatsApp-only card is created with `knowledge_base = NULL` / `business_profile = NULL` (no FB page to seed from) and the client enabled auto-reply immediately after connect — the GA happy path put an ungrounded AI in front of real customers. ⚠️ NOT `isBusinessInfoProvided` (the `kb_filled` activation milestone, ≥80 chars AND ≠ the FB snapshot): measured on prod 2026-07-29, that predicate would refuse 14 of the 39 pages with auto-reply enabled, this one refuses 0. Client shows the reason and opens the Business Info editor; the predicate is deliberately NOT duplicated frontend-side (the server owns the verdict)
  ...
  - **Coexistence** (WhatsApp Business app user onboarding — merchant KEEPS the number on their phone): requested via `extras.featureType='whatsapp_business_app_onboarding'` (read verbatim from Meta's Embedded Signup Builder 2026-07-26; the Builder emits the snake_case URL param `feature_type`, the JS SDK extras key is camelCase). Completion arrives as its own event, `FINISH_WHATSAPP_BUSINESS_APP_ONBOARDING`, alongside `FINISH`/`FINISH_ONLY_WABA`. The connect path **skips Cloud-API `registerPhoneNumber`** for these numbers — registering one would take it off the merchant's phone, the exact thing Coexistence prevents. Persisted as `pages.whatsapp_coexistence` (migration `0140`), set from the event Meta actually sent rather than what was requested (a merchant can switch paths inside the wizard). App-level webhook subscription is `messages,smb_message_echoes,history,smb_app_state_sync,account_update` — Meta requires the first four for the flow to be valid; `account_update` is Meta's only push signal that the WABA↔app link was severed (`PARTNER_REMOVED`, e.g. a coexistence merchant unlinking from their phone: message webhooks stop instantly while the stored token stays valid for weeks — Z net went dark for 27 h this way, 2026-08-31). `webhook.ts → processWhatsAppAccountUpdate` flags every page under the WABA via `markWhatsAppNeedsReconnect` ('app_uninstalled' → reconnect banner + merchant push); all other `account_update` events are logged, never acted on. The admin support console surfaces the same state (the detail payload derives `whatsappNeedsReconnect` the way `serializePage` does): a red "WhatsApp needs reconnecting" pill, an amber dot on the channel badge, and the red health flag `whatsapp_needs_reconnect` — and `isAnyChannelReplying` excludes a severed channel, so token presence alone is never rendered as "the channel works" (the gap that kept the console saying "All good" through the Z net outage). Belt-and-braces for missed events: the 6-hourly token-health sweep also probes the number node itself (`getPhoneNumberInfo` with the merchant token) and flags only on an ALLOWLISTED access-loss Graph code (`isDefinitiveAccessLoss`: 10/33/100/190 + the 200-series permission block) — never on an arbitrary 4xx, because Meta ships rate limiting (4, 80007, 130429) and version deprecations as HTTP 400 too, and a mistaken verdict would banner every number in one sweep. `debug_token` alone cannot see a severed link. An `app_uninstalled` flag does NOT self-clear on a later healthy sweep (the healthy update preserves it via SQL CASE — a probe answering 200 is not proof the link is back); only a real reconnect (`connectWhatsApp`) clears it, while a `token_expired` flag still self-clears when `debug_token` turns healthy. Only `smb_message_echoes` is acted on: each echo becomes an `outgoing` row stored with the `'whatsapp'` platform (so a first-contact echo cannot create a `facebook` conversation on a WhatsApp page). Meta echoes BOTH what the merchant typed and what the WhatsApp Business app sent automatically (greeting / away message), with no author field (verified against Meta's reference, 2026-08-29), so `services/whatsappEchoClassifier.ts` infers authorship from the customer's inbound recency: an echo ≤10 s after an inbound from a customer with no other inbound in the previous 14 days (WhatsApp's own greeting rule; 0/729 human false positives measured) is stored `replyMethod='app_auto'` — no handoff pause, backlog untouched; everything else is `replyMethod='manual'`, which arms the already-shipped handoff pause and marks the customer's backlog answered (D-109). Each echo logs one `whatsapp_echo_classified` line with the verdict and inputs. `history` and `smb_app_state_sync` are accepted and DISCARDED (Meta pushes up to 180 days / thousands of messages — see D-045). Idempotency and the defensive self-send guard are a single lookup on the UNIQUE `messages.platform_message_id`, which carries the real `wamid` since #510. Meta documents that echoes exclude Cloud API messages, so our own replies are never echoed back. The connected card on a Coexistence number tells the merchant to switch the app's Greeting/Away off (every Coexistence vendor's onboarding step) and that a reply from the phone pauses Jawab24 for the handoff window. Requires WhatsApp Business app ≥ 2.24.17 on the merchant's device. **Which path to take is the merchant's answer to one plain question** — `WhatsAppPathModal` ("I already use this number on WhatsApp Business" vs "This number is only for Jawab24"), asked before Embedded Signup because Meta needs `featureType` at popup launch. Asked on a FIRST connect only: `requestConnectWhatsApp` routes a **reconnect** (`page.whatsappConnected === true`, i.e. the expired-token banner) straight through with the number's STORED `whatsappCoexistence`, since re-asking would let a different answer migrate a live coexistence number off the merchant's phone — pinned by two regression tests in `src/__tests__/pages/pages.test.tsx`
...
- **Adapter contract** (`backend/src/integrations/registry.ts`):
  ...
  - **Salla registers SIGNED subscriptions, list-then-upsert (2026-08-23).** `services/salla.ts:registerWebhooks` first reads `GET /admin/v2/webhooks`; an event Salla already holds for our URL is `PUT /webhooks/{id}` with `{version: 2, security_strategy: 'signature', secret: <App Webhook Secret Key>}`, a missing one is `POST /webhooks/subscribe` with the same fields. ⛔ Before this, the subscribe body was `{name, event, url}` only — every subscription sat at `security: {strategy: "", secret: null}`, Salla delivered them with **no `X-Salla-Signature`**, and `verifyWebhookHmac` refused every order/product event with 401 (found on the demo store's first real order; the portal-configured `app.installed` / `app.store.authorize` were the only deliveries ever accepted). A plain re-subscribe answers 422 and would have left them unsigned for good, which is why the repair is an update, and why the per-store "Re-register" button / `scripts/reregister-webhooks.ts` now fix stores connected before the change. **`order.*` events are PORTAL-managed, not API-managed (2026-08-24)**: Salla enforces the app's portal Store Events list for `order.created/updated/status.updated/shipment.created` (`422 «The event type is disabled»` on subscribe AND update since mid-day 2026-08-23) and delivers them app-level for every installed store once the portal list is populated (SALLA_TEST_PLAN.md Tier 0.10). `registerWebhooks` therefore upserts only `SALLA_API_WEBHOOK_EVENTS` (products, `app.uninstalled`, `abandoned.cart`) and DELETES any leftover per-store `order.*` subscription of ours — those pre-enforcement rows are unsigned, unrepairable, and delivered a 401-refused duplicate of every order event. Counting the 422 as a failure had kept `webhookStatus.failed` at 4 forever: retry-until-exhausted + a permanent merchant-facing "Re-register webhooks" CTA.
...
- **Store Facts Sync (D-102, 2026-08-25)**: every `fullSync` also maps the store's contact
  facts from the same `store/info` response — `mobile`+`phone`, `social.whatsapp`,
  `default_branch.working_hours` (Arabic day labels), storefront `domain` as website — into
  every linked page's `pages.business_profile` as provenance source `store_sync`
  (`mapSallaStoreFacts` in `services/salla.ts` → `services/storeFactsSync.ts`). Precedence
  `editor(confirmed) > kb_extract > store_sync > fb_sync`; raw subset snapshotted at
  `platformData.storeFacts`; reaches the AI through the existing BUSINESS_INFO block (no
  PROMPT_VERSION change). Policies are NOT synced — no Salla policy-pages API; the merchant
  authors them in the `/business` editor. Details: `docs/integrations/salla.md`.
- **OAuth Flow**:
...
- **Authorization Modes** (Salla supports two; we implement BOTH):
  ...
  - **Easy Mode** (REQUIRED for published App Store apps): the callback is never hit. Salla pushes tokens server-to-server via the **`app.store.authorize`** webhook (`data: {access_token, refresh_token, expires (unix seconds), scope}`), and re-fires the same event to deliver refreshed tokens.
    - Handler: `controllers/salla.ts:handleStoreAuthorize` (HMAC-verified). Existing ACTIVE store (re-fire) → `updateStoreTokens` (idempotent). Existing DISCONNECTED store (merchant hit "Reauthorize App" after a disconnect) → `reconnectStore` repairs the row in place: tokens, `is_active`, the page links the disconnect severed (`restorePageLinks` reads `platformData.relinkPageIds`, recorded by `disconnectStore`), and webhook registration — the lookup passes `includeInactive: true`, without which the disconnected store is invisible and the reauthorize silently fixes nothing (shipped that way until 2026-08-24). Fresh install → `fetchStoreInfo` for domain/name → `createPendingInstall` keyed by `merchantId` (7-day TTL) — no browser cookie exists.
...
- **Store domain & storefront links (D-097, 2026-08-23)** — learned from the first real Salla conversation:
  - `store/info.domain` is a **full URL**, and for demo/development stores it carries a **path** (`https://demostore.salla.sa/dev-jkgsyu3w6pzzfrzw`). `ecommerce_stores.store_domain` is an identity key (unique with `platform`, the `createStore` ON CONFLICT target), so `fetchStoreInfo` canonicalises it at the border with `services/storeDomain.ts:normalizeStoreDomain` — scheme stripped, host lower-cased, **path kept**, no trailing slash. Every reader builds URLs via `storeBaseUrl`, never by prefixing `https://` inline (the catalog block used to read `https://https://…`). Migration `0177` rewrote the pre-existing rows (stores + pending installs).
  - **Salla products have NO `slug`.** The storefront link is `urls.customer` on the product, persisted as `ecommerce_products.product_url`; `handle` is always NULL for Salla. `services/ecommerce.ts:productUrlFor` is the one resolver (platform-canonical URL first, handle-derived `/products/{handle}` for Shopify/Zid, nothing for Salla without a URL). The former `/p/{slug}` shape was invented and never matched a real store — it left every real Salla row without a link, the RAG chunks without a `URL:` line, `check_inventory` without `productUrl`, and **every Salla product card suppressed** (the card gate keyed on `handle`).
...
- **Billing — Article 5 (Salla-managed billing is MANDATORY for paid apps)**:
  ...
  - ✅ **Salla billing (App Store subscriptions)** — added 2026-08-26, **D-104**. **Verify-first like Zid (D-070)**: Salla documents `GET https://api.salla.dev/admin/v2/apps/{app_id}/subscriptions` (merchant token, `SALLA_APP_ID` required — the rail is dormant without it), so that API is the authority and `app.subscription.*`/`app.trial.*` deliveries are only TRIGGERS calling the one idempotent choke point `syncSallaBilling(storeId)` (`services/sallaBilling.ts`). Four triggers: the subscription/trial webhooks, the uninstall webhook (cancels the mirror), the **post-claim hook** (pre-claim deliveries have no store row to land on and drop by design), and the 6-hourly `SallaBillingReconcile` cron. Mirror lands on `subscriptions` with `payment_method='salla'` + `salla_store_id` (migration `0181`, partial unique over live rows + CHECK); billing subject = workspace owner (shared `resolveBillingSubjectUserId`); a `salla` entry in `LAZY_EXPIRY_CANARIES` fires when everything missed. **Two Salla-specific derivations, [provisional] until the first live paid envelope:** the read has NO status field — entitlement derives from `end_date` (missing/unparseable = `unknown_state`, fail-loud, never "inactive"; null price on an entitled entry = the trial window) — and base plans have NO plan id (name-first mapping, D-103 ex-VAT price 146/296 fallback; `plan_type:'free'` = silent non-entitling; add-on entries never adopted). The suppression above now ALSO fires row-based on the mirror (`isSallaBilled`, canceled exempt) ahead of the Stripe exemption, with the store heuristic kept as the pre-subscription fallback; `buildSallaManageUrl` returns the listing URL once `SALLA_APP_STORE_URL` is set. Coverage: `backend/test/services/sallaBilling.test.ts` + wiring in `backend/test/controllers/salla.test.ts`.
...
- **Webhooks**: registered per store via `POST /v1/managers/webhooks` (body carries `original_id` = `ZID_APP_ID` + a Basic-auth username/password pair); deliveries are verified by **timing-safe HTTP Basic auth** (`utils/basicAuthVerify.ts`) — Zid sends no HMAC signature. Events: `product.create/update/publish/delete`, `order.create`, `order.status.update` (`indelivery`→shipped, `delivered`→delivered). Uninstall arrives as the Partner-Dashboard-configured `app.market.application.uninstall`. **Zid caps subscriptions at 1 per event**, so re-registering an already-connected store (every re-connect and every retry) is normal — it answers the existing events with `409`/`422` **or a `400` whose body says `تم تجاوز الحد الأقصى وهو 1`** ("the maximum, which is 1, has been exceeded"). `services/zid.ts:isZidWebhookAlreadyRegistered` treats all three as already-registered (a `400` for any *other* reason stays a real failure). Same class as Salla's 422 above: counting the duplicate as a failure had left `webhookStatus.failed` non-empty forever → retry-until-exhausted + a false merchant-facing "Re-register webhooks" CTA (confirmed 2026-09-03, `JAWAB24-BACKEND-2P`).
...
- **Store Facts Sync (D-102, 2026-08-25)**: `fullSync` maps the profile's `phone` /
  `mobile_object` / `website`-or-`url` into linked pages' `pages.business_profile` as
...
- **AI Agent Tools** (shared, platform-agnostic — same 5 as Shopify/Salla): `lookup_order`, `track_shipment`, `check_inventory`, `verify_and_get_order`, `verify_and_get_shipment` (whitelist in `packages/shared/src/ecommerce-tools.ts`, executed via `ecommerceActions.ts`). **`check_inventory` resolves in code and answers locally (D-092, 2026-08-22)**: the product is decided by `reply/productResolver.ts` over the page's own product index — a validated `product_id` first, then pg_trgm (exact/near-exact titles), then the reply's reused embedding (article/plural/category phrasings), returning `ambiguous_product` with ≤3 candidates when the lead is not clear and `product_not_found` only below the floor. Stock comes from the synced `ecommerce_products` row; the platform is asked **by id** (`getProductById` — Zid `?id__in=`, Salla `/products/{id}`, Shopify `product(id:)`) only when the row is at/below `LOW_STOCK_UNITS` (5) AND the store's `last_sync_at` is older than `STOCK_REFRESH_MIN` (10, env), never for unlimited (`null`) rows or demo stores; the live figure (count AND status — the status is the sold-out signal for Salla/Zid, so a count written alone left a restocked row reading "out of stock" until the next sync) is written back to the row only and cached at `ecom:stock:{store}:{pid}`; a failed refresh serves the synced figure and reaches Sentry once per store per window. The reply's embedding is reused by the resolver only in `dual`/`off` retrieval mode (the raw message's own); in `enriched` mode the resolver embeds the phrase itself. Every index-stage decision is logged at info (`[ProductResolver] decision`). Sold-out products stay visible as "out of stock" through every reader (`SELLABLE_STATUSES`). The three platform `checkInventory` matchers — and Shopify/Salla's `|| products[0]` first-hit fallback — are gone. Counters: `metrics:ecom:check_inventory:{by_id,id_unknown,by_trigram,by_hybrid,by_title_trigram,embedded,ambiguous,not_found,no_index,local,stale_served,demo_local,live_refresh,live_cached,live_missing,live_failed}`. Calibration: `scripts/product-resolver-probe.ts` → `docs/integrations/product-resolver-probe-2026-08-22.md`.
    **The ORDER tools' Phase 2 is self-healing (D-096, 2026-08-23).** Verification arrives on the customer's NEXT message — a fresh request whose history is text only — so the model re-decides which verify tool to call with no memory of the Phase-1 tool, and in production it crossed families every time (`lookup_order` then `verify_and_get_shipment`, `verification_expired` 4 of 4). `handleVerification` therefore looks for something to verify against in the requesting family's `ecom:pending:{store}:{order|shipment}:{n}` blob (600 s), then the sibling family's, then a **live platform read** of the requested family; the blob is a saved platform call, never a gate. `namesMatch`/`phonesMatch` gate every byte returned — but NOT the read on the no-blob path, where the order must be fetched to be compared against (bounded: the read parks its blob, so repeat guesses on one order number cost nothing more). A sibling-only blob triggers a live read of the requested family, degrading to the sibling data rather than an error. `verification_expired` is retired as unreachable. Counters: `metrics:ecom:verify:{own,sibling,live,requested_empty,requested_live_failed}` — `requested_empty` (200, no shipment yet: healthy) and `requested_live_failed` (threw: where a 403 lands) are separate on purpose; see `docs/SALLA_TEST_PLAN.md` 3.8, which reads them to tell the two apart. Demo stores answer the order tools from `services/demoOrders.ts` constants (placeholder tokens can never reach a platform), which is what lets the eval exercise the flow at all (Cat 81).
...
| Zid | Product sync + KB enrichment (Saudi) | `ZID_*` env vars | 🔧 Live dev-store install + product sync + DMs PROVEN 2026-08-22; order loop (§E) and billing (§H) still unexercised, UI still `coming_soon` (see `docs/integrations/zid.md`, D-020/D-053) |

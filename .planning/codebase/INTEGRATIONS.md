...
- **Connection Flow (implemented — Embedded Signup)**:
  ...
  - `DELETE /pages/:id/whatsapp` disconnects (local-only — no WABA unsubscribe, a WABA can serve multiple numbers); UI: owner-only unlink icon on the connected row → ConfirmationModal. `PATCH /pages/:id/whatsapp-auto-reply` toggles (admin+, plan gate + same billing/trial gates as FB/IG)

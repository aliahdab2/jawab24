...
messageProcessor / commentProcessor  (after reply — fire-and-forget, NEVER awaited)
    └─ maybeCaptureLead()
         ...
         3b. identity turn   this reply's tool round ran verify_and_get_* / find_order_by_phone?
                             → the phone is an ORDER identity claim, not a contact line:
                             count metrics:lead:suppressed:order_verification,

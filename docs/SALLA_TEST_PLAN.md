...
| 3.9 | `app.uninstalled` | store row deactivates; no further webhook processing; **and the billing mirror cancels** (D-104 — no paid local sub outlives the app). If a mirror existed: its `subscriptions` row is `canceled` with `cancel_reason='salla_app_uninstalled'` | a live mirror surviving the uninstall ⇒ the cancel ran after `deactivateStore` blanked the row, or the store never resolved |
| 3.10 | Sentry + health | quiet; `scripts/health-check.sh` green | |

### Tier 3.11 — Billing (D-104). ⛔ THE FIRST LIVE ENVELOPE — capture it, do not assume it

Nothing on this rail has been round-tripped against a live PAID subscription, so the response
shape in `services/sallaBilling.ts` is inferred from docs.salla.dev and marked `[provisional]`.
These rows exist to replace the inference with a capture. **Prerequisite: `SALLA_APP_ID` set in
prod `env/backend.env` + container recreate** — unset, every sync answers `no_store` silently.
...
| 3.11.5 | Cancel/expire the subscription inside Salla | the mirror goes `paused` (not `canceled` — the app is still installed and re-subscribing recovers through the same sync) | `canceled` ⇒ the uninstall path ran instead; `active` ⇒ the end_date comparison never fired |
| 3.11.6 | Capture every payload seen | append each raw envelope to `docs/testing/salla_live_payloads.jsonl` (the Zid rail's file is the precedent) | — |

...
**Technology:**
- Custom `createT()` function in `src/i18n/translations.ts`
- Flat JSON files with dot-notation keys (`src/i18n/en.json`, `src/i18n/ar.json`)
- Next.js locale routing for URL-based language switching
- RTL (Right-to-Left) support for Arabic via CSS logical properties
...
No external i18n libraries needed — Jawab24 uses a lightweight custom setup.
...
Both files are **flat JSON** with dot-notation keys (879 keys each):
```json
{
  "common.save": "Save",
  "common.cancel": "Cancel",
  "dashboard.title": "Dashboard",
  "settings.businessHours.start": "Start Time"
}
```
...
See `frontend/docs/TRANSLATION_GUIDE.md` for full rules (key naming, RTL, brand terms).
...
### Frontend
- [x] Custom i18n with `createT()` and `useTranslation()` hook
- [x] 879 translation keys in both `en.json` and `ar.json`
- [x] Language toggle in header (Next.js locale routing)
- [x] RTL support with CSS logical properties
- [x] Automated validation script (`npm run translation:validate`)
- [ ] Test all screens in both languages (visual regression)

### Backend
- [x] Implement language detection
- [x] Add language fields to database
- [x] Template selection by language
- [x] API responses in user language

### AI Worker
- [x] Create bilingual system prompts
- [x] Add Syrian dialect prompt
- [x] Test with Syrian phrases
- [x] Implement language detection

### Templates
- [x] Create bilingual templates
- [x] Add Syrian dialect examples
- [x] Test keyword matching (both languages)

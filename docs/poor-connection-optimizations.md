...
- `public/brand/og-social.png` - Reduced from 408KB to 50KB
  - *Superseded 2026-08-07:* now generated at 1200x630 by `npm run social:generate` (68KB).
...
> ⚠️ **The `.backup` files are no longer in the repo** — these two `mv` commands will fail.
> Restore from git history instead (`git show <rev>:frontend/public/brand/logo-main.png`).
>
> ⚠️ **`og-social.png` is no longer a hand-optimized asset** (since 2026-08-07). It is
> generated by `npm run social:generate` (`frontend/scripts/generate-social-images.ts`) and
> must be regenerated, never hand-edited or restored from a backup — the old backup is a
> 1024x1024 JPEG whose artwork names only Facebook and Instagram, which is the bug that
> made the shared-link preview omit WhatsApp for weeks after launch.

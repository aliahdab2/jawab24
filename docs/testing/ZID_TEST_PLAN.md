...
| L-18 | 🧪 pinned | `frontend/src/__tests__/pages/pages.test.tsx` «Facebook connect inside a platform frame» — 5 cases, mutation-checked ×3. Found live 2026-08-30 on the dev store: «ربط قناة» / «إعادة الاتصال» on `/pages` navigated the *iframe* to facebook.com → «www.facebook.com refused to connect». ☐ live capture owed: run it in the frame after deploy. ⚠️ Finish the Facebook step within **15 min** — the scoped session's cookie expires then (pre-existing seam, see `docs/integrations/zid.md`) |
